name Donald Galbraith
birth -date 1784 -location ireland -note 'son of Edward Galbraith'
death -date 20-jun-1859 -location gigha -ref donald1784-death
tags ireland gigha greenock
external {
    familysearch GHYS-8TB
    findagrave 149745010
}
note {
    Unknown why Janet was referred to as Janet Pool and Janet McLean, however confirmed it is the same person due to her daughter's death in 1882.  Mother's maiden name is given as "Poole McLean."
}

partner {
    name Janet Poole McLean
    marriage -date 'about 1807' -location Ireland
    birth -date 'about 1781'
    death -date 23-nov-1841 -location gigha

    child {
        name Margaret Peggy Galbraith
        birth -date 1808 -location 'Ballymoney, County Antrim, Ireland'
        death -date 31-jan-1893 -location gigha -ref margaret1808-death -note unmarried
    }
    child {
        name Janet Galbraith
        birth -date 'about 1811' -location gigha -note 'unrecorded'
        death -date 09-jul-1882 -location gigha -ref janet1811-death
        partner {
            name Donald Taylor
        }
    }
    child {
        name Donald Galbratih
        baptism -date 07-may-1813 -location gigha -ref donald1813-baptism
        death -date 27-sep-1890 -location campbeltown -ref donald1813-death
        partner {
            name Catherine Whiteford[?]
        }
    }
    child {
        name Mary Galbraith
        birth -date 30-nov-1815 -location gigha -ref mary1815-baptism
        partner {
            name Donald McConachie
        }
    }
    child {
        name Alexander Galbraith
        baptism -date 28-jul-1819 -location gigha -ref alexander1819-baptism
        death -date 11-aug-1886 -location gigha -ref alexander1819-death -note unmarried
    }

    child galbreath-peter-1822-niven
}
body {
Isle of Gigha Census 1827, North Drimachro:$ref[census1827]
$csvtable{
First,Last,Age,Year
Donald,Galbreath,40,1787
Janet,Pool,42,1785
Peggy,Galbraith,18,1809
Janet,Galbraith,15,1812
Donald,-,14,1813
Mary,-,10,1817
Alexander,-,6,1821
Peter,-,5,1822
}
Donald was listed as a weaver.
}
body {
Census of 1841, South Ardenlag, Gigha, Argyllshire, Scotland:$ref[census1841]
$csvtable{
First,Last,Age,Year,Place
Donald,Galbreath,50,1791,Ireland
Janet,Galbreath,50,1791,Ireland
Donald,Galbreath,25,1816,Argyllshire
Mary,Galbreath,20,1821,Argyllshire
Alexander,Galbreath,22,1819,Argyllshire
Peter,Galbreath,22,1823,Argyllshire
Betty,McNiel,5,1836,Argyllshire
Anhus,Clark,1,1840,Argyllshire
}
Donald was listed as Woolen & linen weaver.
}


footnotes {

    donald1784-death {
        $sp-ref-link[d-1859-537-000-0005 0002 "Donald Galbraith"]{
        Name: Galbraith, Donald, Bell-man[?], (widower);
        Date: 1859 June Twentieth;
        Age: 75 years [birth ~1784];
        Father: Edward Galbraith, Shoemaker (deceased)
        Mother: [not listed];
        Burial: Kildalton,Gigha
        }
    }
    margaret1808-death {
        $sp-ref-link[d-1893-537-00-0004 0002 "Margaret Galbraith"]
    }
    janet1811-death {
        $sp-ref-link[d-1882-537-00-0004 0002 "Janet Galbraith"]
    }
    donald1813-baptism {
        $opr-ref[b-1813-537-000-0010-0050 "Donald Galbreath"]
    }
    donald1813-death {
        $sp-ref-link[d-1890-507-00-0124 0042 "Donald Galbraith"]
    }
    mary1815-baptism {  
        $opr-ref[b-1815-537-000-0010-0053 "Mary Galbreath"]
    }
    alexander1819-baptism {
        $opr-ref[b-1819-537-000-0010-0062 "Alexander Galbreath"]
    }
    alexander1819-death {
        $sp-ref-link[d-1886-537-00-0008 0003 "Alexander Galbraith"]
    }
    census1827 {
        https://www.gigha.org.uk/viewItem.php?id=8932
    }
    census1841 {
        https://www.findmypast.com/transcript?id=GBC/1841/0016662256&expand=true
    }
}
