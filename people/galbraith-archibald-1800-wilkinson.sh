name Archibald Galbraith
tags 'argyll::Killean and Kilchenzie' Argyll:Campbeltown
baptism -date 29-oct-1800 -location 'killean and kilchenzie' -ref archibald1800-baptism
death -date 13-feb-1855 -location campbeltown -ref archibald1800-death
todo {
    cleanup census stuff
}
todo {
    a few sons were married but died young.  Children TBD.
}
note {
    Wilkinson is angelicized version of McQuilkan.  Maybe her parents:
        maybe parents:
    MCQUILKAN
DUNCAN
MARGARET MCMATH/FR351 (FR351)
06/06/1799
519
10 / 341
KILLEAN AND KILCHENZIE

}

partner {
    name Barbara Wilkinson
    marriage -date 1827 -location 'killean and kilchenzie'
    death -date 13-mar-1886 -location 'mid calder, midlothian' -ref barbara-death 
    body {
        Maybe the daughted of Duncan McQuilkan and Margaret McMath, baptized 6 Jun 1799 in Killean and Kilcehnzie.
    }
    child {
        name Duncan Galbreath
        baptism -date 17-jan-1828 -location 'killean and kilchenzie' -ref duncan1828-baptism
    }
    child {
        name Archibald Galbreath
        baptism -date 29-apr-1830 -location campbeltown -ref archibald1830-baptism
    }
    child {
        name Alexander Galbreath
        baptism -date 11-Nov-1832 -location campbeltown -ref alexander1832-baptism
        death -date 3-nov-1875 -location denningstoun,lanark -ref alexander1832-death
        partner {
            name Margaret McArthur
        }
        body {
            Died in the poorhouse with fractured ribs "due to violence." Children, marriage TBD.
        }
    }
    child {
        name Angus Galbreath
        baptism -date 14-Feb-1835 -location campbeltown -ref angus1835-baptism
    }
    child {
        name Margaret Galbraith
        baptism -date 6-Jul-1837 -location campbeltown -ref margaret1837-baptism
        partner {
            name William Wilson
        }
    }
    child {
        name Archibald Galbreath
        baptism -date 22-Aug-1839 -location campbeltown -ref archibald1839-baptism
        death -date 10-may-1857 -location 'milton,glasgow' -ref archibald1839-death -note unmarried
    }
    child galbraith-angus-1841-lindsay
}
body {
   $p{
    Barbara may have been name McQuilkan, which is somewhat similar to Wilkinson.
    Perhaps an Angelized version.

    }
    $ul{
        $li{1851 Archibald listed as Mashman: https://www.findmypast.com/transcript?id=GBC/1851/0019257311&expand=true}
        $li{1861 - Barbra https://www.findmypast.com/transcript?id=GBC%2F1861%2F0023675920}
        $li{1871 - Barbra https://www.findmypast.com/transcript?id=GBC%2F1871%2F0024730641}
    }
}
footnotes {
    archibald1800-baptism {
        $opr-ref-link[b-1800-519-000-0010-0148 "Archibald Galbreath"]
    }
    archibald1800-death {
        $sp-ref-link[d-1855-507-01-0011 0004 "Archibald Galbraith"]{
        11. Archibald Galbraith, Mashman
        Death: 1855 February Thirdteeth, Age 56
        Father: Duncan Galbraith, Laboroer;
        Mother: Mary Galbraith, Maiden Name Kelly;
        Spouse: Barbara Galbraith, Maiden Name Wilkenson;
        Present: Alex Galbraith, son;
        $i{list of children, other details}
        }
    }
    barbara-death {
        $sp-ref-link[d-1886-694-00-0010 0004 "Barbara Galbraith"]{
        10. Barbara Galbraith, Widow of Archibald Galbraith, Brewer;
        Death: 1886 March Thirdteenth, age 86 [$i{birth 1800}];
        Father: Duncan Wilkinson, farmer, deceased;
        Mother: Margaret Wilkison, M.S. Mathieson, deceased;
        Present: Jane Galbraith, daughter in law;
        }
    }
    duncan1828-baptism {
        $opr-ref[b-1828-519-000-0020-0066 "Duncan Galbreath"]
        Mother idnexed as  "Barbra McCuilkan"
    }
    archibald1830-baptism {
        $opr-ref-link[b-1830-507-000-0060-0203 "Archibald Galbreath"]
        Mother indexed as "Barbra McQuilkan"
    }
    alexander1832-baptism {
        $opr-ref-link[b-1832-507-000-0070-0018 "Alexander Galbreath"]
    }
    alexander1832-death {
        $sp-ref-link[d-1875-644-03-1699 0567 "Alexander Galbraith"]
        correction present.. unclear what changed.
    }
    angus1835-baptism {
        $opr-ref[b-1835-507-000-0070-0057 "Angus Galbreath"]
    }
    margaret1837-baptism {
        $opr-ref-link[b-1837-507-000-0070-0105 "Margaret Galbreath"]
    }
    archibald1839-baptism {
        $opr-ref-link[b-1839-507-000-0070-0154 "Archibald Galbreath"]
    }
    archibald1839-death {
        $sp-ref-link[d-1857-644-07-0338 0113 "Archibald Galbraith"]
    }
}
