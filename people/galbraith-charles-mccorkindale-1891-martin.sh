name Charles McCorkindale Galbraith
tags Argyll:Campbeltown
birth -date 1891 -location campbeltown -ref charles1891-birth
death -date 24-dec-1926 -location 'Kilmore and Kilbride,Argyll' -ref charles1891-death
partner {
    name Christina Martin
    birth -date 'about 1891'
    marriage -date 04-jun-1912 -location Southend -ref martin-marriage

    child {
        name Christina Martin Galbraith
        birth -date 17-apr-1917 -location campbeltown -ref christina1917-birth
    }
}

footnotes {

    charles1891-birth {
        $sp-ref[b-1891-507-00-0017 "Charles McCorkindale Galbraith"]
    }
    charles1891-death {
        $sp-ref[d-1921-523-00-0110 "Charles Galbraith"]
    }
    martin-marriage {   
        $sp-ref-link[m-1912-532-00-0001 0001 "Charles McCorkindale Galbraith" "Christina Martin"]
    }
    christina1917-birth {
        $sp-ref-link[b-1917-507-00-0052 0018 "Christina Martin Galbraith"]
    }
}
