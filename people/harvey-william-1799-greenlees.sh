name William Harvey
gender male
tags 'Argyll:Campbeltown' immigrant:usa:illinois:boone
external {
    familysearch K4RC-26Q
    wikitree Harvie-840
    findagrave 53401630
    ancestry tree/12079504/person/282663072015
}
birth -date 04-jan-1799 
baptism -date 06-jan-1799 -location campbeltown -ref william1799-baptism
death -date 22-jun-1872 -location caledonia,boone,illinois,usa
body {
    He was a tanner.
}
body {
    Immigrations before 17-jun-1841.  It appears they missed the Census of 1841
}
partner {
    name Mary Greenlees
    gender female
    birth -date 28-aug-1807
    baptism -date 30-aug-1807 -location campbeltown
    death -date 16-jul-1868 -location caledonia,boone,illinois,usa

    marriage -date 03-jul-1827 -location campbeltown
    body {
         She was the daughter of John Greenlees and Anne Ferguson.
    }
    child {
        name William Harvey
        gender male
        baptism -date 14-apr-1828 -location campbeltown
        death -date 23-nov-1909 -location ' Rockford, Winnebago County, Illinois,usa'
        partner {
            name Elizabeth Harvie
            gender female
            marriage -date 09-aug-1870 -location southend
        }
    }
    child {
        name Matthew Harvey
        gender male
        birth -date 17-jan-1836 
        baptism -date 19-jan-1836 -location campbeltown
        death -date 11-sep-1901 -location rockford,winnebago,illinois,usa
        partner {
            name Jane Ralston
            gender female
            baptism -date 3-feb-1848 -location campbeltown
            death -date 17-jan-1925 -location rockford,winnebago,illinois,usa
            body {
                She was the daughter of Alexander Ralston and Margaret McKerral.
            }       
        }
    }
    child {
        name Robert Harvey
        gender male
        baptism -date 19-oct-1837 -location campbeltown -ref robert1837-baptism
    }
    child {
        name Margaret Harvey
        gender female
        baptism -date 22-mar-1840 -location campbeltown -ref margaret1840-baptism
    }
    child {
        name Daniel Greenlee Harvery
        gender male
        baptism -date 25-oct-1850 -location argyle,winnebago,illinois,usa
        death -date 12-feb-1939 -location rockford,winnebago,illinois,usa
        body {
            Author of $source-link[argyle-settlement]{The Argyle Settlement in History and Story}.
        }
    }
}
footnotes {
    william1799-baptism {
        $opr-ref[b-1799-507-000-0040-0178 "William Harvie"]
    }
    robert1837-baptism {
        $opr-ref-link[b-1837-507-000-0070-0112 "Robert Harvey"]{

        }
    }
    margaret1840-baptism {
        $opr-ref-link[b-1840-507-000-0070-0169 "Margaret Harvey"]{
            

        }
    }
}
