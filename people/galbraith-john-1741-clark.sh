name John Galbreath
tags Argyll:Campbeltown 'dead end'
external {
    familysearch KZCJ-WYC
}
birth -date 'say 1741'
note {
There is another $child-link[galbraith-john-1730-clark]{John Galbreath} who married a Jean Clark.
}
partner {
    name Catharine Clark
    marriage -date 01-jul-1766 -location campbeltown -ref clark-marriage

    child {
        name Jean Galbraith
        baptism -date 31-may-1767 -location campbeltown -ref jean1767-baptism
    }
    child {
        name John Galbreath
        baptism -date 11-jul-1769 -location campbeltown -ref john1769-baptism
    }
    child {
        name Peter Galbreath
        baptism -date 27-mar-1772 -location campbeltown -ref peter1772-baptism
    }
    child {
        name James Galbraith
        baptism -date 10-jul-1775 -location campbeltown -ref james1775-baptism
    }
}
footnotes {
    clark-marriage {
        $opr-ref-link[m-1766-507-000-0011-0467 "John Galbraith" "Catharine Clerk"]
        $opr-ref-link[m-1766-507-000-0010-0300 "John Galbraith" "Catharine Clerk"]
    }
    jean1767-baptism {
        $opr-ref[b-1767-507-000-0011-0358 "Jean Galbreath"]
        $opr-ref[b-1767-507-000-0010-0316 "Jean Galbraith"]
    }
    john1769-baptism {
        $opr-ref[b-1769-507-000-0011-0371 "Peter Galbreath"]
        $opr-ref[b-1769-507-000-0010-0334 "Peter Galbraith"]
    }
    peter1772-baptism {
        $opr-ref-link[b-1772-507-000-0011-0383 "Peter Galbreath"]
    }
    james1775-baptism {
        $opr-ref-link[b-1775-507-000-0020-0020 "John Galbreath"]{
            James | Lawful son to John Galbreath and 
            Katharine Clark. Born 10th July Bapt the same Day
        }
    }
}
