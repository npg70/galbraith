name Archibald Galbraith
tags 'Killean and Kilchenzie' 'Campbeltown'
birth -date 17-jan-1865 -location 'killean and kilchenzie' -ref archibald1865-birth
death -date 27-nov-1949 -location Campbeltown -ref archibald1865-death
burial -name 'Kilkerran Cemetery'
external {
    findagrave 179781210
}
note {
    The first child Flora was born before marriage and originally listed has
    "Flora Robertson."  However after the marriage there is a correction, saying
    the birth had been "legitimatized."
}
partner {
    name Margaret Robertson
    marriage -date 1-jun-1894 -location 'killean and kilchenzie' -ref robertson-marriage
    death -date 22-apr-1950

    child {
        name Flora Galbraith
        birth -date 21-aug-1893  -location 'killean and kilchenzie' -ref flora1893-birth
        death -date 7-mar-1993
    }

    child galbraith-elizabeth-mcmillan-1895-mccormack

    child {
        name Archibald Galbraith
        birth -date 1897 -location 'killean and kilchenzie' -ref archibald1897-birth
        death -date 17-jul-1979
    }
    child {
        name Margaret Morrison Galbraith
        birth -date 6-mar-1899 -location campbeltown -ref margaret1899-birth
    }
    child {
        name Jessie Campbell Galbraith
        birth -date 1901 -location campbeltown -ref jessie1901-birth
        death -date 30-mar-1993
    }
    child { 
        name Hector Robertson Galbraith
        birth -date 1906 -location campbeltown -ref hector1906-birth
        death -date 29-nov-1989
    }
}

footnotes {
    archibald1865-birth {
        $sp-ref-link[b-1865-519-00-0008 0003 "Archibald Galbraith"]
    }
    archibald1865-death {
        $sp-ref-link[d-1949-507-00-0101 0034 "Archibald Galbraith"]
    }
    robertson-marriage {
        $sp-ref-link[m-1894-519-00-0001 0001 "Archibald Galbraith" "Margaret Robertson"]
    }
    flora1893-birth {
        $sp-ref-link[b-1893-519-00-0021 0007 "Flora Galbraith"] "Flora Robertson" in the record since the birth occured before the marriage.
    }
    archibald1897-birth {
        $sp-ref[b-1897-519-00-0004 "Archibald Galbraith"]
    }
    margaret1899-birth {
        $sp-ref-link[b-1899-507-00-0072 0024 "Margaret Morrison Galbraith"]
    }
    jessie1901-birth {
        $sp-ref[b-1901-507-00-0079 "Jessie Campbell Galbraith"]
    }
    hector1906-birth {
        $sp-ref[b-1906-507-00-0260 "Hector Robertson Galbraith"]
    }
}
