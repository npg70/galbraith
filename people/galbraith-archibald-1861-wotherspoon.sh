name Archibald Galbraith
tags 'kilcalmonell and kilberry'
external {
    familysearch LV7Y-5RK
}

birth -date 27-may-1861 -location kilberry -ref archibald1861-birth
death -date 29-jan-1940 -location kilberry -ref archibald1861-death
partner {
    name Helen Wotherspoon
    birth -date 8-aug-1875 -location gigha
    death -date 2-jun-1951 -location kilberry -ref wotherspoon-death
    marriage -date 26-aug-1897 -location 'kilcalmonell and kilberry' -ref wotherspoon-marriage
    child {
        name James Galbraith
        birth -date 11-mar-1895 -location kilberry  -ref james1895-birth
        death -date 25-sep-1951 -location 'Swift Current, Saskatchewan, Canada'
    }
    child {
        name Malcolm Galbraith
        birth -date 23-dec-1897 -location kilberry -ref malcolm1897-birth
        death -date 12-jan-1968 -location mull,argyll -ref malcolm1897-death
    }
    child {
        name Christina Galbraith
        birth -date 23-nov-1899 -location kilberry -ref christina1899-birth
        death -date 23-jun-1984 -location 'maybole,argyll' -ref christina1899-death
    }
    child {
        name Lachlan Galbraith
        birth -date 30-jan-1902 -location kilberry -ref lachlan1902-birth
        death -date 5-nov-1949 -location kilberry -ref lachlan1902-death
    }
    child {
        name Eugina Galbraith
        birth -date 29-jun-1904 -location kilberry -ref eugina1904-birth
        death -date 1978 -location ayr,ayr 
    }
    child {
        name Archibald Wotherspoon Galbraith
        birth -date 25-mar-1907 -location kilberry -ref archibald1907-birth
        death -date 18-dec-1970 -location 'Torinturk, Argyll, Scotland' -ref archibald1907-death
    }
    child {
        name Helen Galbraith
        birth -date 12-dec-1909 -location kilberry -ref helen1909-birth
        death -date jan-2008 -location lochgilphead -ref helen1909-death
    }
    child {
        name Jessie Mary Galbraith
        birth -date 18-aug-1912 -location  kilberry -ref jessie1912-birth
        death -date 17-jul-1927 -location glasgow -ref jessie1912-death
        burial -name 'Kilnaish Burial Ground' -location kilmartin
    }
    child {
        name Martha Wotherspoon Galbraith
        birth -date 16-jul-1915 -location kilberry -ref martha1915-birth
        death -date 24-sep-1915 -location kilberry -ref martha1915-death
    }
    child {
        name Catherine Galbraith
        birth -date 8-sep-1916 -location kilberry -ref catherine1916-birth
        death -date 5-aug-2009 -location glasgow -ref catherine1916-death
    }
    child {
        name William Wotherspoon Galbraith
        birth -date 20-sep-1920 -location tarbert -ref william1920-birth
        death -date 27-mar-1977 -location 'Cinderford, Gloucestershire, England'
        partner {
            name Joyce Elizabeth Thomas
            birth -date 24-feb-1921 -location 'Cinderford, Gloucestershire, England'
            death -date 13-nov-2011
        }
    }

}
footnotes {
    archibald1861-birth {
        $sp-ref[b-1861-516-02-0004 "Archibald Galbreath"]
    }
    archibald1861-death {
        $sp-ref-link[d-1940-533-02-0002 0001 "Archibald Galbraith"]
    }
    wotherspoon-marriage {

    }
    wotherspoon-death {

    }
    james1895-birth {
        $sp-ref[b-1895-533-02-0005 "James Galbraith"]
    }
    malcolm1897-birth {
        $sp-ref-link[b-1898-533-02-0001 0001 "Malcolm Galbraith"] Note born in 1897 but registered in 1898.
    }
    christina1899-birth {
        $sp-ref[b-1899-533-02-0006 "Christina Galbraith"]
    }
    lachlan1902-birth {
        $sp-ref-link[b-1902-533-02-0002 0001 "Lachlan Galbraith"]
    }
    eugina1904-birth {
        $sp-ref[b-1904-533-02-0006 "Eugina Galbraith"]
    }
    archibald1907-birth {
        $sp-ref[b-1907-533-02-0003 "Archibald Wotherspoon Galbraith"]
    }
    helen1909-birth {
        $sp-ref[b-1909-533-02-0009 "Helen Galbraith"]
    }
    jessie1912-birth {
        $sp-ref[b-1912-533-02-0006 "Jessie Mary Galbraith"]
    }
    martha1915-birth {
        $sp-ref[b-1915-533-02-0004 "Martha Wotherspoon Galbraith"]
    }
    catherine1916-birth {
        $sp-ref[b-1916-533-02-0004 "Catherine Galbraith"]
    }
    william1920-birth {
        $sp-ref-link[b-1920-533-02-0005 0002 "William Wotherspoon Galbraith"]
    }
}
