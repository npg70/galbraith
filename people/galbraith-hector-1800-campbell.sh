name Hector Galbraith
birth -date 'about 1800'
tags 'Kilcalmonell and Kilberry'
external {
    familysearch KHT9-74M
}
body {
    He was a cooper.
}
note {
    Death likely before 1855.  Once source says birth was 1798 and death was 1854.
}
note {
   His wife used Ann, Agnes, Nancy as first names. 
}
todo {
    Sons, Robert and Archibald, both married.  At least one had children.
}
partner {
    name Ann Campbell
    marriage -date 23-jun-1821 -location 'Kilcalmonell and Kilberry' -ref campbell-marriage

    child {
        name Katherine Galbraith
        baptism -date 10-sep-1821 -location 'Kilcalmonell and Kilberry'  -ref katherine1821-baptism
    }
    child {
        name Angus Galbraith
        baptism -date 8-feb-1826 -location 'Kilcalmonell and Kilberry'  -ref angus1826-baptism
    }
    child {
        name Mary Galbraith
        baptism -date 8-feb-1826 -location 'Kilcalmonell and Kilberry' -ref mary1826-baptism
    }
    child {
        name Dugald Galbraith
        birth -date 3-nov-1828
        baptism -date 3-nov-1828 -location 'Kilcalmonell and Kilberry' -ref dugald1828-baptism
        death -date 4-feb-1881 -location 'Kilberry' -ref dugald1828-death -note 'umd'
    }
    child {
        name Robert Galbraith
        birth -date 2-jan-1831
        baptism -date 8-jan-1831 -location 'Kilcalmonell and Kilberry' -ref robert1831-baptism
        death -date 25-may-1896 -location Paisley -ref robert1831-death
    }
    child {
        name John Galbraith
        birth -date 10-apr-1833
        baptism -date 27-apr-1833 -location 'Kilcalmonell and Kilberry' -ref john1833-baptism
    }
    child {
        name Hecter Galbraith
        baptism -date 5-apr-1835 -location  'Kilcalmonell and Kilberry' -ref hector1835-baptism
    }
    child {
        name Jean Galbraith
        baptism -date 08-nov-1837 -location 'Kilcalmonell and Kilberry' -ref jean1837-baptism
    }
    child {
        name Archibald Galbraith
        baptism -date 18-may-1840 -location 'Kilcalmonell and Kilberry' -ref archibald1840-baptism
    }

    child galbraith-archibald-1842-mctaggart

}

footnotes {

    campbell-marriage {
        $opr-ref-link[m-1821-516-000-0020-0202 "Hector Galbraith" "Ann Campbell"]{
            The which day were booked in order to marriage
            Hector Galbraith and Ann Campbell both in this
            parish of Kilcalmonell and Kilberry.
        }
    }
    katherine1821-baptism {
        $opr-ref[b-1821-516-000-020-0020 "Katherine Galbraith"]
    }
    angus1826-baptism {
        $opr-ref[b-1826-516-000-020-0302 "Angus Galbraith"]
    }
    mary1826-baptism {
        $opr-ref[b-1826-516-000-020-302 "Mary Galbraith"]
    }
    dugald1828-baptism {
        $opr-ref-link[b-1828-516-000-020-0312 "Dugald Galbraith"]
    }
    dugald1828-death  {
        $sp-ref-link[d-1881-516-02-0002 0001 "Dugald Galbraith"]
    }
    robert1831-baptism {
        $opr-ref-link[b-1831-516-000-020-0321 "Robert Galbraith"]
    }
    robert1831-death {
        $sp-ref-link[d-1896-573-00-0561 0187 "Robert Galbraith"]
    }
    john1833-baptism {
        $opr-ref-link[b-1833-516-000-020-0333 "John Galbraith"]
    }
    hector1835-baptism {
        $opr-ref[b-1835-516-000-020-0339 "Hecter Galbraith"]
    }
    jean1837-baptism {
        $opr-ref-link[b-1837-516-000-020-0346 "Jean Galbraith"]
    }
    archibald1840-baptism {
        $opr-ref[b-1840-516-000-020-350 "Archibald Galbraith"]
    }
}
