name Susanna Galbreath
gender female
tags 'argyll:killean and kilchenzie'
external {
    familysearch K28L-RDH
    findagrave 167549500
}

baptism -date 10-aug-1809 -location 'killean and kilchenzie' -ref susanna1809-baptism
death -date 9-Mar-1888  -location campbeltown -ref susanna1809-death

partner {
    name William Blue
    marriage -date 19-apr-1831 -location 'killean and kilchenzie' -ref blue-marriage
    birth -date 16-sep-1802
    baptism -date 17-sep-1802 -location campbeltown
    death -date 9-oct-1847 -location campbeltown

    child {
        name William Blue
        baptism -date 1832 -location campbeltown
        death -date 1890
    }
    child {
        name Duncan Blue
        baptism -date 1835 -location campbeltown
    }
    child {
        name Archibald Blue
        baptism -date 1840 -location campbeltown
        death -date 1899
    }
    child {
        name Alexander Blue
        baptism -date 1846 -location campbeltown
        death -date 1902
    }
}
footnotes {
    susanna1809-baptism {
        $opr-ref-link[b-1809-519-000-0010-0229 "Susanna Galbreath"]
    }
    susanna1809-death {
        $sp-ref-link[d-1888-507-000-0045 0015 "Susanna Galbraith"]
    }
    blue-marriage {
        Registered twice.  First:
        $opr-ref-link[m-1809-519-000-0010-0225 "Susie Galbreath" "William Blue" ]{
        April 19 | Will.m Blue Campltn \\ Susie Galbreath Ross...[?]
        },
        Second: $opr-ref-link[m-1809-507-000-0000-0300 "Susan Galbreath" "William Blue" ]{
            William Blue, Slater of this Parish and Susan Galbreath
            of Killean Parish were married 20 April 1831.
        }
    }
}
