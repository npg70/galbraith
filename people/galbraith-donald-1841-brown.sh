name Donald-Duncan Galbraith
tags kildalton
external {
    familysearch M9L5-2W3
}
baptism -date 19-sep-1841 -location kildalton -ref donald1841-baptism
death -date 20-dec-1908 -location kildalton -ref donald1841-death
note {
    Parents were Malcolm Galbraith and Rachel McLellan.
    https://www.familysearch.org/tree/person/details/MG8J-JDV

    Possible this is two people.
}
body {
    He was a distillery worker / maltman.
}
partner {
    name Catherine Brown
    marriage -date 31-jul-1873 -location greenock -ref brown-marriage

    child {
        name Margaret Galbraith
        birth -date 28-jul-1874 -location kildaton -ref margaret1874-birth
        death -date 27-mar-1895 -location kildaton -ref margaret1874-death
        body {
            Unmarried.
        }
    }
    child {
        name Malcolm Galbraith
        birth -date 1876 -location kildaton -ref malcolm1876-birth
        death -date 2-oct-1952 -location paisley,renfrew -ref malcolm1876-death
        partner {
            name Ann Graham
        }
        partner {
            name Mary D. MacLean
        }
        body {
            Detective Inspector
        }
    }
    child galbraith-hugh-1879-mckay

    child {
        name John Galbraith
        birth -date 11-nov-1881 -location kildaton -ref john1881-birth
        death -date 2-mar-1963 -location 'Alexandria, Dunbartonshire, Scotland' -ref john1881-death
        partner {
            name Mary Brown
            child  {
                name Hugh B Galbraith
            }
        }
    }
    child {
        name Alexander Galbraith
        birth -date 1884 -location kildaton -ref alexander1884-birth
        death -date 19-feb-1938 -location govan -ref alexander1884-death
        partner {
            name Allison Keith Millar
        }
    }
}
footnotes  {
    donald1841-baptism {
        Unclear why two entries, one appears to an different index.
        $opr-ref-link[b-1841-531-000-0020-0054 "Donald Galbraith"]
        $opr-ref-link[b-1841-541-000-0020-0076 "Donald Galbraith"]
    }
    donald1841-death {
        $sp-ref-link[d-1908-541-00-0038 0013 "Duncan Galbraith"]
    }
    brown-marriage {
        $sp-ref-link[m-1873-564-03-0246 0123 "Duncan Galbraith" "Catherine Brown"]
    }
    margaret1874-birth {
        $sp-ref[b-1874-541-00-0036 "Margaret Galbraith"]
    }
    margaret1874-death {
        $sp-ref-link[d-1895-541-00-0018 0006 "Margaret Galbraith"]
    }
    malcolm1876-birth {
        $sp-ref[b-1876-541-00-0025 "Malcolm Galbraith"]
    }
    malcolm1876-death {
        $sp-ref-link[d-1952-573-01-0968 0323 "Malcolm Galbraith"]
    } 
    john1881-birth {
        $sp-ref[b-1881-541-00-0053 "John Galbraith"]
    }
    john1881-death {
        $sp-ref-link[d-1963-493-00-0101 0034 "John Galbraith"] 
    }
    alexander1884-birth {
        $sp-ref[b-1884-541-00-0056 "Alexander Galbraith"]
    }
    alexander1884-death {
        $sp-ref-link[d-1938-644-17-0213 0071 "Alexander Galbraith"]
    }
}


