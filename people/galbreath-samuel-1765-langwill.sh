name Samuel Galbreath
birth -date 'about 1765'
body {
Little is now about the origins of Samuel. Guessing his birth around 1765 would make first child born when he was 28 which seems reasonable. All children appear to have been born "in Town" and the marriage said the couple were "both of this parish".
}
partner {
    name Janet Langwill
    marriage -date 1792
    child {
        name Archibald Galbreath
        baptism -date 24-nov-1793 -location campbeltown -ref archibald1793-baptism
    }
    child {
        name Janet Galbreath
        baptism -date 26-oct-1795 -location campbeltown -ref janet1795-baptism
    }
    child galbreath-david-1797-mitchell
    child galbreath-martha-1800-mcmillan
    child {
        name James Galbreath
        baptism -date 15-sep-1802 -location campbeltown -ref james1802-baptism
    }
    child galbreath-mary-1804-mcgill
    child {
        name Agnes Galbreath
        baptism -date 3-sep-1806 -location campbeltown -ref agnes1806-baptism
    }
}
footnotes {
    archibald1793-baptism {
        $opr-ref-link[b-1793-507-000-0040-0111 "Archibald Galbreath"]
    }
    janet1795-baptism {
        $opr-ref-link[b-1795-507-000-0040-0131 "Janet Galbreath"]
    }
    james1802-baptism {
        $opr-ref-link[b-1802-507-000-0040-0234 "James Galbreath"]
    }
    agnes1806-baptism {
        $opr-ref-link[b-1806-507-000-0040-0292 "Agnes Galbreath"]
    }
}
