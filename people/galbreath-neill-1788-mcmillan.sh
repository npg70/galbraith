name Neill Galbreath
tags Argyll:Campbeltown
baptism -date 18-dec-1788 -location campbeltown -ref neill1788-baptism
death -date 9-jun-1863 -location campbeltown -ref neill1788-death
external {
    familysearch M1TB-RY9
}
partner {
    name Mary McMillan
    marriage -date 26-may-1810 -location gorbals -ref mcmillan-marriage
    birth -date 'about 1785'
    death -date 24-jan-1866 -location campbeltown -ref mcmillan-death
    child {
        name Anne Galbreath
        baptism -date 30-mar-1811 -location campbeltown -ref anne1811-baptism
    }
    child {
        name John Galbreath
        baptism -date 27-jul-1813 -location campbeltown -ref john1813-baptism
    }
    child galbraith-sarah-1817-mccallum
    child {
        name Neill Galbreath
        baptism -date 22-dec-1819 -location campbeltown -ref neill1819-baptism
    }
    child {
        name Mary Galbreath
        baptism -date 19-apr-1821 -location campbeltown -ref mary1821-baptism
    }
    child galbreath-hugh-1823-mcnair
    child {
        name Janet Galbreath
        baptism -date 22-dec-1825 -location campbeltown -ref janet1825-baptism
    }
    child {
        name Neill Galbreath
        baptism -date 19-feb-1829 -location campbeltown -ref neill1829-baptism
    }
}
footnotes {
    neill1788-baptism {
        $opr-ref-link[b-1788-507-000-0040-0078 "Neill Galbreath"]
    }
    neill1788-death {
        $sp-ref-link[d-1863-507-00-0083 0028 "Neil Galbraith"]
    }
    mcmillan-death {
        $sp-ref-link[d-1866-507-00-0009 0003 "Mary Galbraith"]
    }
    mcmillan-marriage {
        $opr-ref-link[m-1810-644-002-0030-0153 "Neil Galbreath" "Mary McMillan"]{
            May 1810 26th | Neil Galbreath + Mary McMillan both in this Parish ...    
        }
    }
    anne1811-baptism {
        $opr-ref[b-1811-507-000-0040-0367 "Anne Galbreath"]
        Mother indexed as "MacMillan"
    }
    john1813-baptism {
        $opr-ref[b-1813-507-000-0040-0400 "John Galbreath"]
        Mother indexed as "MacMillan"
    }
    neill1819-baptism {
        $opr-ref[b-1819-507-000-0040-0502 "Neill Galbreath"]
    }
    mary1821-baptism {
        $opr-ref[b-1821-507-000-0060-0033 "Mary Galbreath"]
    }
    janet1825-baptism {
        $opr-ref[b-1825-507-000-0060-0116 "Janet Galbrath"]
    }
    neill1829-baptism {
        $opr-ref[b-1829-507-000-0060-0177 "Neill Galbreath"]
    }
}
