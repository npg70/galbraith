name John Galbreath
birth -date 04-jul-1773 -location Scotland
tags Gigha
external {
    familysearch LRD8-6SV
}
todo {
    parents are known and moved to USA.
}
partner {
    name Isobel McQuilkan

    child {
        name Angus Galbreath
        baptism -date 11-feb-1805 -location gigha -ref angus1805-baptism
    }
    child {
        name Donald Galbreath
        baptism -date 13-oct-1806 -location gigha -ref donald1806-baptism
    }
    child {
        name Malcolm Galbraith
        baptism -date 28-may-1808 -location gigha -ref malcolm1808-baptism
    }
    child {
        name Catharine Galbreath
        baptism -date 05-jun-1810 -location gigha -ref catharine1810-baptism
        body {
            maybe death in 1813
        }
    }
    child {
        name Hugh Galbreath
        baptism -date 05-jun-1812 -location gigha -ref hugh1812-baptism
        body {
            maybe death in 1813
        }
    }
    child {
        name Lachlan Galbreath
        baptism -date 05-nov-1814 -location gigha -ref lachlan1814-baptism
        death -date 11-nov-1814 -location gigha -ref lachlan1814-death
    }
    child galbraith-john-1816-mcgeachy

}
body {
Isle of Gigha Census 1827, North Drimachro:$ref[census1827]
$csvtable{
First,Last,Age,Year
John, Galbraith, 46, 1781
Bell, Macquilkan, 50, 1777
Angus, Galbraith, 23, 1804
Donald, Galbraith, 20, 1807
Malcolm, Galbraith, 19, 1808
John, Galbraith, 11, 1816
}
}

footnotes {
    census1827 {
        https://www.gigha.org.uk/viewItem.php?id=8932
    }
    angus1805-baptism {
        $opr-ref-link[b-1805-537-000-0010-0029 "Angus Galbreath"]
    }
    donald1806-baptism {
        $opr-ref-link[b-1806-537-000-0010-0032 "Donald Galbreath"]
    }
    malcolm1808-baptism {
        $opr-ref[b-1808-537-000-0010-0038 "Malcom Galbreath"]
    }
    catharine1810-baptism {
        $opr-ref[b-1810-537-000-0010-0043 "Catharine Galbreath"]
    }
    hugh1812-baptism {
        $opr-ref-link[b-1812-537-000-0010-0048 "Hugh Galbreath"]
    }
    lachlan1814-baptism {
        $opr-ref-link[b-1814-537-000-0010-0052 "Lachlan Galbreath"]
    }
    lachlan1814-death {
        $opr-ref[d-1814-537-000-0010-0054 "Lachlan Galbreath"]
    }
}
