name John Galbreath
baptism -date 07-oct-1799  -location gigha -ref john1799-baptism
death -date 25-aug-1876 -location gigha -ref john1799-death
tags Argyll:Gigha
external {
    familysearch LDHS-FMN 
    findagrave 170505471
}
note {
    There is no census for Gigha in 1841.
}
todo {
    Census 1851: https://www.findmypast.com/transcript?id=GBC/1851/0019321970&expand=true
}
todo {
    Census 1861: https://www.findmypast.com/transcript?id=GBC/1861/0022539054&expand=true
}
todo {
    Census 1871: https://www.findmypast.com/transcript?id=GBC/1871/0023498590&expand=true
}

partner {
    name Effy McQuilkan
    birth -date 06-mar-1811 -location 'Killean and Kilchenzie' -note 'daughter of Malcolm McQuilkan and Ann McCormaig'
    death -date 11-may-1890 -location gigha -ref mcquilkan-death
    marriage -date 09-apr-1835 -location gigha -ref mcquilkan-marriage
    child {
        name Malcolm Galbreath
        baptism -date 10-feb-1837 -location gigha -ref malcolm1837-baptism
        death -date 20-sep-1887 -location glasgow -ref malcolm1837-death -note unmarried
        body {
            Farmer.
        }
    }
    child {
        name Sarah Galbraith
        birth -date 02-jan-1839 -location gigha -ref sarah1839-baptism
        death -date 22-jan-1899 -location gigha -ref sarah1839-death -note unmarried
    }
    child {
        name Anne Galbraith
        baptism -date 09-apr-1842 -location gigha -ref anne1842-baptism
        death -date 14-dec-1921 -location gigha -ref anne1842-death
        partner {
            name John Greenlees
        }
    }
    child {
        name Margaret Galbraith
        baptism -date 21-apr-1844 -location gigha -ref margaret1844-baptism
        death -date 03-sep-1894 -location glasgow -ref margaret1844-death
        partner {
            name Angus Wilkinson
        }
    }
    child {
        name John Galbraith
        baptism -date 07-jul-1846 -location gigha -ref john1846-baptism
        death -date 09-jun-1925 -location gigha -ref john1846-death -note unmarried
        body {
            Marine Engineer.
        }
    }
    child {
        name Betsy Galbraith
        birth -date 12-jun-1848 -location gigha -ref betsy1848-baptism
        death -date 11-jan-1914 -location gigha -ref betsy1848-death -note unmarried
    }
    child {
        name Euphemia Galbraith
        birth -date 06-nov-1850 -location gigha -ref euphemia1851-baptism
        death -date 09-mar-1926 -location gigha -ref euphemia1851-death -note unmarried

    }
    child galbreath-archibald-1854-mcbain
}
footnotes {
    john1799-baptism {
        $opr-ref[b-1799-537-000-0010-0017 "John Galbreath"]
    }
    john1799-death {
        $sp-ref-link[d-1876-537-00-0002 0001 "John Galbraith"]
    }
    mcquilkan-death {
        $sp-ref-link[d-1890-537-00-0004 0002 "Effy Galbraith"]
    }
    mcquilkan-marriage {
        $opr-ref-link[m-1835-537-000-0020-0031 "John Galbreath" "Effie McQuilkan"]
    }
    malcolm1837-baptism {
        $opr-ref[b-1837-537-000-0020-0035 "Malcolm Galbreath"]
    }
    malcolm1837-death {
        $sp-ref-link[d-1887-644-10-0629 0210 "Malcolm Wilkison Galbraith"]
    }
    sarah1839-baptism {
        $opr-ref[b-1839-537-000-0020-0040 "Marion Galbreath"]. Note that "Sarah" was commonly used for "Merran" and "Marion"
    }
    sarah1839-death {
        $sp-ref-link[d-1899-537-00-0001 0001 "Sarah Galbraith"]
    }
    anne1842-baptism {
        $opr-ref[b-1842-537-000-0020-0049 "Anne Galbreath"]
    }
    anne1842-death {
        $sp-ref-link[d-1921-537-00-0005 0002 "Anne Galbraith"]
    }
    margaret1844-baptism {
        $opr-ref[b-1844-537-000-0020-0056 "Margaret Galbreath"]
    }
    john1846-baptism {
        $opr-ref[b-1846-537-000-0020-0062 "John Galbreath"]
    }
    john1846-death {
        $sp-ref-link[d-1925-537-00-0002 0001 "John Galbraith"]
    }
    betsy1848-baptism {
        $opr-ref-link[b-1848-537-000-0020-0067 "Betsey Galbraith"]
    }
    betsy1848-death {
        $sp-ref-link[d-1914-537-00-0001 0001 "Betsy Galbraith"]
    }
    euphemia1851-baptism {
        $opr-ref[b-1850-537-000-0020-0072 "Effy Galbraith"]
    }
    euphemia1851-death {
        $sp-ref-link[d-1926-537-00-0001 0001 "Euphemia Galbraith"]
    }
}
