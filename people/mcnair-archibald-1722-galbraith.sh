name Archibald McNair
gender male
birth -date 'about 1722'
tags Argyll:Campbeltown
external {
    familysearch LV5G-R9S
    wikitree McNair-303
}
note {
    Margaret's origins is unknown.
}
note {
Margaret is sometimes confused with $child-link[galbreath-mary-1729-armour]{Mary Galbreath}, born 1729, daughter of $child-link[galbreath-william-1701-kilpatrick/]{William Galbreath} and Margaret Jean Kilpatrick, who married William Armour.
}

partner {
    name Margaret Galbraith
    gender female
    birth -date 'say 1727' -ref margaret-birth
    marriage -date 06-dec-1747 -location campbeltown -ref marriage

    child {
        name John McNair
        gender male
        baptism -date 18-jul-1750 -location campbeltown
        partner {
            name Janet Brody
        }
    }
    child {
        name Mary McNair
        gender female
        baptism -date 20-apr-1752 -location campbeltown
        partner {
            name James Mitchell
            gender male
        }
    }
    child {
        name Agnes McNair
        gender female
        baptism -date 10-mar-1754 -location campbeltown
        death -date 1831 -location 'Belfast,County Antrim,Northern Ireland'
        partner {
            name Edward Armour
            gender male
        }
    }
    child {
        name James McNair
        gender male
        baptism -date 13-jun-1756 -location campbeltown
        partner {
            name Susanna Martin
            gender female
        }
    }
    child {
        name Peter McNair
        gender male
        baptism -date 30-apr-1758 -location campbeltown
        partner {
            name Jean McLeran
            gender female
        }
    }
    child mcnair-archibald-1759-mitchell
    child {
        name Robert McNair
        gender male
        baptism -date 05-mar-1760 -location campbeltown
        death -note dy
    }
    child {
        name Robert McNair
        gender male
        baptism -date 11-nov-1761 -location campbeltown
        death -date 1824 -location 'Black Cape,Bonaventure, Quebec, Canada'
    }
    child {
        name Nathaniel McNair Sr
        gender female
        baptism -date 06-jan-1763 -location campbeltown
        death -date 13-sep-1857 -location 'New Mills,New Brunswick,Canada'
        partner galbreath-margaret-1771-mcnair
    }
    child {
        name Elizabeth McNair
        gender female
        birth -date 12-may-1765
        baptism -date 16-may-1765 -location campbeltown
    }
    child {
        name Janet McNair
        gender female
        birth -date 12-may-1765
        baptism -date 16-may-1765 -location campbeltown
    }
    child {
        name Margaret McNair
        gender female
        baptism -date 08-feb-1767 -location campbeltown
    }
    child {
        name Ann McNair
        gender female
        birth -date 16-nov-1768
        baptism -date 24-nov-1768 -location campbeltown
    }
}
footnotes {
    margaret-birth {
        Margaret had children over an 18 year period.  Assuming the last child happened when she was 40, gives us a birth year of 1728.  This implies she got married around age 20, which seems about right. 
    }
    marriage {
        $opr-ref-link[m-1747-507-000-0011-0450 "Archibald McNair" "Margaret Galbraith"]{
        [1747] Archibald McNair & Margaret Galbraith | Dec 6th
        }
        $opr-ref[m-1747-507-000-0010-0187 "Archibald McNair" "Margaret Calbraith"]
    }
}
