name John Galbraith
gender male
tags usa:pennsylvania:Allegheny ireland
birth 'about 1730' -location 'Killymore, Ardstraw, Tyrone, Northern Ireland'
death -date 18-aug-1800 -location Pennslyvania,USA
external {
    familysearch L7B8-4KF
    cgs 270108974297
}
note {
    First wife is unknown.
}
note {
    Second wife Agnes is frequently listed as "Agnes Langwell" or "Nancy Agnes Langwell" with out sources.  Langwell may have been a guess to match with the John of Campbeltown below.
}
confused-with galbreath-john-1721-langwill {
     This John in 1795 listed 15 living children in his will, and none were named $child-link[galbreath-alexander-1753-flemming]{Alexander}
}
confused-with galbreath-john-1736-mary-ann 
body {
Will and Probate:$ref[probate1]$ref[probate2]
$blockquote{
   No. 88 In the name of God Amen J John Galbraith of Allegheny County
   and State of Pennsylvania ... my son William ... my son James ... my son David ...
  ... my daughter Martha ... my daughter Elizabeth ... my daughter Jean ... 
 my dear wife Agnes ... my other nine children John, Samuel, Mary, Sarah, Robert,
Agness, Thomas, Rebecka, and Rachel.   ... my son David'd wife Margaret West ...
applied my seal this tenth day of October one thousand seven hundred + ninety five
}
$p{
This is followed with a statement:
}
$blockquote{
Henry County, Clerks Office Aug 18th 1800. I Do hereby certify that a true copy
of this will is lodged in my office.  Test. Ron Thomas.
}
$p{
That is Henry County, Kentucky.  Certainly his daughter Martha spent time in Henry County, and William, James, and David supposed
lived in Henry as well.  It seems John moved there or was visiting when he died.
}
$p{
The will is proved (witnesses confirming they saw the will signed) in Alegheny,PA a few months later on 27 Nov 1800.
}
}

partner {
    name _____ _____

    child {
        name William Galbraith
    }
    child {
        name James Galbraith
    }
    child {
        name David Galbraith
        gender male
        partner {
            name Margaret West
            gender female
        }
    }
    child {
        name Martha Galbraith
        gender female
        birth -date 27-dec-1770 -location allegheny,pennsylvania,usa
        death -date 10-mar-1848 -location henry,kentucky,usa
        partner {
            name _____ Parks
            gender male
        }
        partner {
            name Elija Sutton
            gender male
            birth -date 09-aug-1772 -location lancaster,virginia,usa
            death -date 14-aug-1857 -location 'new castle,henry,kentucky,usa'
            marriage -date 1793 -location Washington,Pennsylvania,USA
        }
    }
    child {
        name Elizabeth Galbraith
    }
    child {
        name Jean Galbraith
    }
}
    
partner {
    name Agnes _____
    birth -date 1745
    death -date 1803

    child {
        name John Galbraith
    }
    child {
        name Samuel Galbraith
    }
    child {
        name Mary Galbraith
        gender female
        partner {
            name Daniel Calelazer
            gender male
            marriage -date 15-jan-1797
        }
    }
    child {
        name Sarah Galbraith
    }
    child {
        name Robert Galbraith
    }
    child {
        name Agnes Galbraith
    }
    child {
        name Thomas Galbraith
    }
    child {
        name Rebecca Galbraith
    }
    child {
        name Rachel Galbraith
        gender female
        birth -date 'about 1795'
        partner {
            name John Colclasure
            gender male
            marriage -date 'about 1814'
        }
    }
}

footnotes {
    probate1 {
        Will Books, Vol 1-2, 1789-1823, Case Number 88, Allegheny, Pennsylvania, USA on 
        $elink[https://www.ancestry.com/search/collections/8802/records/1814756]{Ancestry.com}, and  originally indexed as "John Gulbraith":
        $blockquote{
            Source Citation
Will Books, 1789-1917; Author: Allegheny County (Pennsylvania). Register of Wills
        }
        $blockquote{
Source Information
Ancestry.com. Pennsylvania, U.S., Wills and Probate Records, 1683-1993 [database on-line]. Lehi, UT, USA: Ancestry.com Operations, Inc., 2015.
        }
        $blockquote{
Original data: Pennsylvania County, District and Probate Courts.
        }
    }
    probate2 {
        Will Packets, Vol 1, 1789-1808, Case Number 88, Allegheny, Pennsylvania, USA  on
        $elink[https://www.ancestry.com/search/collections/8802/records/4602275]{Ancestry.com}.
        $blockquote{

            Will Packets Or Files, 1789-1917; Author: Allegheny County (Pennsylvania). Register of Wills
        }
        $blockquote{
Description
Notes: Will Packets, Vol 1, 1789-1808
        }
        $blockquote{
Source Information
Ancestry.com. Pennsylvania, U.S., Wills and Probate Records, 1683-1993 [database on-line]. Lehi, UT, USA: Ancestry.com Operations, Inc., 2015.
        }
        $blockquote{
Original data:Pennsylvania County, District and Probate Courts.
        }
}
}

