name John Galbraith
baptism -date 13-may-1804 -location campbeltown -ref birth
death -date 12-jul-1871 -location campbeltown -ref death
external {
    familysearch 9M9H-VCZ
}
body {
    His occupation was in the distilling industry.
}
partner {
    name Janet Watson
    marriage -date 11-dec-1838 -location campbeltown -ref marriage
    child {
        name Jane Galbraith
        baptism -date 22-dec-1839 -location campbeltown -ref jane-birth
    }
    child {
        name John Galbreath
        baptism -date 26-sep-1842 -location campbeltown -ref john-birth
    }
    child {
        name Janet Galbreath
        baptism -date 25-may-1845 -location campbeltown -ref janet-birth
    }
    child {
        name Isabella Galbraith
        baptism -date 8-sep-1850 -location campbeltown -ref isabella-birth
    }
    child {
        name William Galbraith
        baptism -date 7-nov-1853 -location campbeltown -ref william-birth
    }
}

footnotes {
    birth {
        $opr-ref-link[b-1804-507-000-0040-0262 "John Galbraith"]
    }
    death {
        $sp-ref-link[d-1871-507-00-0090 0030  "John Galbraith"]{
            90. John Galbraith, widow of Janet Watson. 1871 July Twelveth. Age 71.
            Father: William Galbraith, Distillery Workman (deceased),
            Mother: Jane Galbraith M.S. Campbell (deceased),
            Jeanie Speirs Daughter, present.
        }
    }
    marriage {
        $opr-ref-link[m-1838-507-000-0060-0348 "John Galbreath" "Janet Watson"]
    } 
    jane-birth {
        $opr-ref-link[b-1839-507-000-0070-0163 "Jane Galbreath"]
    }
    john-birth {
        $opr-ref-link[b-1842-507-000-0070-0226 "John Galbreath"]
    }
    janet-birth {
        $opr-ref-link[b-1845-507-000-0070-0284 "Janet Galbreath"]
    } 
    isabella-birth {
        $opr-ref[b-1850-507-000-0070-0373 "Isabella Galbraith"]
    }   
    william-birth {
        $opr-ref-link[b-1853-507-000-0070-0420 "William Galbraith"]

        Note: mother is indexed as Janet Wilson.
    }
}
