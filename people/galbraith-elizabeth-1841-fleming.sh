name Elizabeth Galbraith
gender female
tags Argyll:Campbeltown
birth -date 25-apr-1841
baptism -date 10-may-1841 -location campbeltown -ref elizabeth1841-baptism
death -date 10-jan-1931 -location campbeltown -ref elizabeth1841-death
todo major { cleanup html, finish children }
note {
    extensive notes on FindAGrave
}
external {
    familysearch M1TD-WMJ
    findagrave 167874666
}
partner {
    name James Nicol Fleming
    gender male
    marriage -date 27-oct-1859 -location campbeltown 
    body {
        He was director at the City of Glasgow Bank, which collpased in 1878. He was convicted of fraud, served 8 months in prison, and died in Campbeltown in 1904.
    }
    child {
        name John Galbraith Fleming
        gender male
    }
    child {
        name Mary H Fleming
        gender female
    }
    child {
        name Jane E Fleming
        gender female
    }
    child {
        name Archibald N Fleming
        gender male
    }
    child {
        name Elizabath L Fleming
        gender female
    }
    child {
        name Margaret J Fleming
        gender female
    }
    child {
        name Andrew Galbraith Fleming
        gender male
    }
}
note {
    see orig https://github.com/npg70/galbreath-old/blob/main/people/galbraith-elizabeth-1841.md
}

body {
1878 - rumoured to have fled to Spain.
1882 - Convicted
1904 - Died
After his conviction, there were issue with him improving the house that was in trust for wife, using effectively stolen monies.4

Interestingly, there was also a ship "James Nicol Fleming", launched in 1869. The ships owner was Albion Shipping, owned by James Galbraith of Glasgow5. In 1879, the name was changed to "Napier" (also a name related to the Galbraiths).

Prior to this James Galbraith, launched the ship "Margaret Galbraith" in 1868,6 named after this wife.

Just a conciendence that Fleming's wife was a Galbraith, and the Galbraith shipbuilder named his ship after Fleming? I assumed Fleming approved the loan for the ship.

1881 Census https://www.findmypast.com/transcript?id=GBC/1881/0017950203&expand=true -- Living with her brother Archibald Galbreath.

}
footnotes {
    elizabeth1841-baptism {
        $opr-ref-link[b-1841-507-000-0070-0196 "Elizabeth Galbreath"]
    }
    elizabeth1841-death {
        $sp-ref-link[d-1931-507-00-0005 0002 "Elizabeth Galbraith"]
    }
    marriage {
        $sp-ref-link[m-1859-507-00-0033 0017 "Elizabeth Galbraith" "I N Fleming"]{
            J N Fleming, age 27, residence Bombay, India, Merchant / Bachelor,
            Father: John Fleming (deceased); Mother: (not listed)
        }
        (Should be "J N Fleming")
    }
}

