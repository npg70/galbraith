name Archibald Galbreath
birth -date 'about 1794'
death -date 15-feb-1859 -location Saddell -ref archibald1794-death 
tags 'Argyll:Kilcalmonell and Kilberry'
external {
    familysearch KCFP-591
}

partner {
    name Mary McLean
    marriage -date 20-jan-1821 -location 'Kilcalmonell and Kilberry' -ref mclean-marriage

    child galbraith-neil-1823-mckinnon
    child {
        name Ann Galbreath
        baptism -date 25-apr-1826 -location 'Kilcalmonell and Kilberry' -ref ann1826-baptism
    }
    child {
        name Allan Galbraith
        baptism -date 20-aug-1828 -location 'Kilcalmonell and Kilberry' -ref allan1828-baptism
    }

    child galbraith-donald-1833-mcdermid
}

partner {
    name Margaret Walker
    baptism -date 5-jul-1805 -location 'Kilcalmonell and Kilberry' -note 'Daughter of Niel Walker and unknown mother'

    marriage -date 27-dec-1833 -location Kilberry -ref walker-marriage

    child {
        name Duncan Galbreath
        baptism -date 21-dec-1834 -location 'Kilcalmonell and Kilberry' -ref duncan1834-baptism
        death -date 5-aug-1875 -location Greenock -ref duncan1834-death -note umn
    }
    child {
        name Mary Galbreath
        baptism -date 10-feb-1836 -location 'Kilcalmonell and Kilberry' -ref mary1836-baptism
    }
    child galbraith-archibald-1838-crawford
}

footnotes {
    archibald1794-death {
        $sp-ref-link[d-1859-531-01-0002 0001 "Archibald Galbraith"]
    }
    mclean-marriage {
        $opr-ref-link[m-1821-516-000-0020-0200 "Archibald Galbreath" "Mary McLean"]
    }
    ann1826-baptism {
        $opr-ref[b-1826-516-000-0020-0303 "Ann Galbraith"]
    }
    allan1828-baptism {
        $opr-ref[b-1828-516-000-0020-0311 "Allan Galbraith"]
    }
    walker-marriage {
        $opr-ref-link[m-1834-516-000-0020-0386 "Archibald Galbreath" "Margret Walker"]{
            1st January 1834 The which day were booked in order to Marriage
            Archd Galbreath $i{illegible} and Margret Walker in Acheon[?] both in this
            parish and proclamed on the 29 Dec 1833
        }

        $opr-ref-link[m-1833-533-000-0020-0214 "Archibald Galbreath" "Margaret Walker"]{
            27 Dec 1833 Registered for marriage Archd Galbreath in the parish of
            Clachan + Mary Walker in this parish
        }
    }

    duncan1834-baptism {
        $opr-ref[b-1834-516-000-0020-0338 "Duncan Galbreath"]
    }
    duncan1834-death {
        $sp-ref-link[d-1875-564-03-0707 0236 "Duncan Galbraith"]
    }
    mary1836-baptism {
        $opr-ref[b-1836-516-000-0020-0342 "Mary Galbreath"]
    }
}
