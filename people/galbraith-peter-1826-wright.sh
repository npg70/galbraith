name Peter Galbraith
gender male
external {
    familysearch K4XV-8MX
}
todo error { one child is listed twice. both can not be correct.  }
tags ayr:dalrymple argyll:campbeltown
birth -date 1-apr-1826
baptism -date 9-apr-1826 -location dalrymple -ref peter1826-baptism
death -date 10-dec-1902 -location 'Shropshire, England'
partner {
    name Lilias Wright
    gender female

    marriage -date 16-may-1853 -location dalrymple
    birth -date 2-may-1832 -location 'St Quivox, Ayrshire, Scotland, United Kingdom'
    death -date 3-dec-1903 -location 'Shropshire, England, United Kingdom'

    child {
        name William Galbraith
        gender male
        birth -date 10-jun-1858 -location campbeltown -ref william1858-birth
    }
    child {
        name David Wright Galbraith
        gender male
        birth -date 18-mar-1860 -location campbeltown -ref david1860-birth
    }
    child {
        name Lilias Wright Galbraith
        gender male
        birth -date 28-jan-1862 -location campbeltown -ref lilias1862-birth
        death -date oct-1890 -location 'Newport, Shropshire, England, United Kingdom'
    }
    child {
        name Jane Galbraith
        gender male
        birth -date 6-feb-1864 -location campbeltown -ref jane1864-birth
    }
    child {
        name John Galbraith
        gender male
        birth -date 17-apr-1865 -location campbeltown -ref john1865-birth
        death -date 14-jul-1914 -location 'Newport, Shropshire, England, United Kingdom'
    }
    child {
        name Peter Galbraith
        gender male
        birth -date 18-jan-1868 -location campbeltown -ref peter1868-birth
        death -date 10-mar-1869 -location campbeltown -ref peter1868-death
    }
    child {
        name Peter Galbraith
        gender male
        tags immigrant
        birth -date 22-jan-1871 -location campbeltown -ref peter1871-birth
        death -date 24-jul-1927 -location 'Queensland, Australia'
        partner {
            name Ada Gertrude Murray
            marriage -date apr-1905 -location 'Newport, Shropshire, England, United Kingdom'
        }
    }
}
footnotes {
    peter1826-baptism {
        $opr-ref[b-1826-588-000-0030-0027 "Peter Galbraith"]
    }
    william1858-birth {
        $sp-ref-link[b-1858-507-00-0101 0034 "William Galbraith"]
    }
    david1860-birth {
        $sp-ref[b-1860-507-00-0073 "David Wright Galbraith"]
    }
    lilias1862-birth {
        $sp-ref[b-1862-507-00-0034 "Lilias Wright Galbraith"]
    }
    jane1864-birth {
        $sp-ref[b-1864-507-00-0052 "Jane Galbraith"]
    }
    john1865-birth {
        $sp-ref-link[b-1865-507-00-0071 0024 "John Galbraith"]
    }
    peter1868-birth {
        $sp-ref-link[b-1868-507-00-0025 0009 "Peter Galbraith"]
    }
    peter1871-birth  {
        $sp-ref-link[b-1871-507-00-0024 0008 "Peter Galbraith"]
    }
        
}
