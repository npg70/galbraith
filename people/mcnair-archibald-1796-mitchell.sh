name Archibald McNair
gender male
tags Argyll:Campbeltown Immigrant:USA:Ohio:Clermont
birth -date 23-nov-1796 -location Chisken
baptism -date 25-nov-1796 -location campbeltown -ref archibald1796-baptism
death -date 25-jul-1864 -location monroe,clermont,ohio,usa
external {
    ancestry tree/12079504/person/282657232244
    familysearch K4TR-T61
    findagrave  123641931
}
body {

He was a farmer in Lochsanish.

Immigration in 1842 on British Barque "Gleaner" arrived at New York on 5 July 1842, 33 days from Campbeltown, Scotland:$ref[gleaner]

$csvtable{
#, First, Last, Age, Year, Gender, Role
81, Arch, McNair, 43, 1799, M, farmer
82, Jean, Mitchell, 36, 1806, F, sister-in-law
83, Arch, McNair, 13, 1829, M, son
84, Hugh, McNair, 10, 1832, M, son
85, Jn, McNair, 8, 1834,  M, son
86, Jean, McNair, 6, 1836, F, daughter
87, Robt, McNair, 4, 1838, M, son
88, James, McNair, 2 1/2, 1840, M, son
}

Mary appears to have died after the last child James was born in 1841 and before landing in New York in 1842.  In her place, her sister $child-link[mitchell-jean-1806]{Jean} is listed on the ship's manifesto.

After landing, they moved to Clermont County, Ohio.

}
note {
    Mary Mitchell is often confused with $child-link[mitchell-mary-1803-greenlees]{Mary Mitchell} who married Robert Greenlees.
}
note {
    Mary Mitchell has been historically confused with $child-link[mitchell-mary-1799-sheddan]{Mary Mitchell}, b. 1799, m. James Sheddan.
}
note {
    Possible additional children  Mary and Robert, born in Ohio. (impossible).
}

partner {
   name Mary Mitchell
   gender female
   parent mitchell-john-1773-langwill 
   marriage -date 28-apr-1825 -location campbeltown -ref mitchell-marriage
   birth -date 10-Apr-1802 -location campbeltown
   death -date 'about 1841'

   child {
        name Archibald McNair
        gender male
        birth -date 02-jan-1827
        baptism -date 11-jan-1827 -location campbeltown -ref arch1827-baptism
   }
   child {
        name John McNair
        gender male
        baptism -date 10-feb-1829 -location campbeltown -ref john1829-baptism
        death -note dy
    }
   child {
        name Hugh McNair
        gender male
        baptism -date 04-jul-1830 -location campbeltown -ref hugh1830-baptism
    }
   child {
        name John McNair
        gender male
        baptism -date 26-apr-1832 -location campbeltown -ref john1832-baptism
    }
   child {
        name Jane McNair
        gender female
        baptism -date 07-apr-1834 -location campbeltown -ref jane1834-baptism
    }
   child {
        name Janet McNair
        gender female
        birth -date 30-jun-1837
        baptism -date 06-jul-1837 -location campbeltown -ref janet1837-baptism
    }
    child {
        name Robert McNair
        gender male
        baptism -date 13-nov-1838 -location campbeltown -ref robert1838-baptism
    }
    child {
        name James McNair
        gender male
        baptism -date 28-mar-1841 -location campbeltown -ref james1841-baptism
    }
}
footnotes {
    archibald1796-baptism {
        $opr-ref-link[b-1796-507-000-0040-0144 "Archibald McNair"]{
            Archibald Lawful son to Archd McNair + Jean Mitchel on Chisken born 23rd Bapt 25th
        }
    }
    mitchell-marriage {
        $opr-ref-link[m-1825-507-000-0060-0267 "Archibald McNair" "Mary Mitchell"]{
            Archibald McNair Farmer Cheskan + Mary Mitchell daughter
            of John Mitchell Farmer Kilkivan married 28 Sept 1825
        }
    }
    arch1827-baptism {
        $opr-ref-link[b-1827-507-000-0060-0152 "Archibald McNair"]{
            Arch | Lawful son of Arch McNair + Mary Mitchell
            in Lochsanish was born 2nd + bapt 11 Janry Arch
        }
    }
    john1829-baptism {
        $opr-ref[b-1829-507-000-0060-0177 "John McNair"]
    }
    hugh1830-baptism {
        $opr-ref[b-1830-507-000-0060-0207 "Hugh McNair"]
    }
    john1832-baptism {
        $opr-ref[b-1832-507-000-0070-0006 "John McNair"]
    }
    jane1834-baptism {
        $opr-ref[b-1832-507-000-0070-0043 "Jane McNair"]
    }
    janet1837-baptism {
        $opr-ref-link[b-1837-507-000-0070-0105 "Janet McNair"]{
            $i{july 1837} Janet | Lawful daughter of Archibald McNair
            farmer Lochsanish and Mary Mitchell was born
            thirtieth June + Bapt 6 July
        }
    }
    robert1838-baptism {
        $opr-ref[b-1838-507-000-0070-0134 "Robert McNair"]
    }
    james1841-baptism {
        $opr-ref[b-1841-507-000-0070-0193 "James McNair"]
    }
    gleaner {
        $elink[http://www.ralstongenealogy.com/glnrlist#mcsporan]{Ralston Geneaology}
    }   
}
