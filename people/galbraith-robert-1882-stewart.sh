name Robert Galbraith
tags Campbeltown
birth -date "about 1882" -location campbeltown -ref robert1883-birth

external {  
    familysearch L5JJ-TC9
}

partner {
    name Catherine Hyndman Stewart
    birth -date 'about 1879' -note 'daughter of Gilbert Stewart and Mary Hart'
    marriage -date 9-dec-1904 -location campbeltown -ref stewart-marriage
    death -date 20-aug-1960 -location campbeltown -ref stewart-death

    child {
        name Mary Hart Galbraith
        birth -date 17-mar-1905 -location campbeltown
    }
    child {
        name Robert Galbraith
        birth -date 2-dec-1906 -location campbeltown
    }
    child {
        name Janet Galbraith
        birth -date 1909 -location campbeltown -ref janet1909-birth
        death -date 16-jul-1965 -location southend -ref janet1909-death
        partner {
            name William Robertson
        }
        partner {
            name James Beattie
        }
    }
}
footnotes {
    robert1883-birth {
        Appears unrecorded.
    }
    stewart-marriage {
        $sp-ref-link[m-1904-507-00-0061 0031 "Robert Galbraith" "Kate Stewart"]
    }
    stewart-death {
        $sp-ref-link[d-1960-507-00-0063 0021 "Catherine Hyndm Galbraith"]
    }
    janet1909-birth {
        $sp-ref[b-1909-507-00-0079 "Janet Galbraith"]
    }
    janet1909-death {
        $sp-ref-link[d-1965-532-00-0004 0002 "Janet Galbraith"]
    }
}
