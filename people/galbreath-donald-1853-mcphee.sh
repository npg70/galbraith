name Donald Galbreath
tags Colonsay
baptism -date 5-may-1853 -location colonsay -ref donald1853-baptism
death -date 23-feb-1907 -location colonsay -ref donald1853-death
todo {
    Add 1891 census info 
    https://www.findmypast.com/transcript?id=GBC/1891/0035244081&expand=true
}

partner {
    name Christina McPhee
    birth -date 'about 1852'
    death -date 30-aug-1898 -location colonsay -ref mcphee-death
    marriage -date 30-may-1879 -location Greenock -ref donald1853-marriage
    child galbraith-malcolm-1879-mcdonald
    child {
        name Hugh Galbraith
        birth -date 26-apr-1882 -location greenock -ref hugh1882-birth
        death -date 25-dec-1962 -location glasgow -ref hugh1882-death
        partner {
            name Marion "Smith or McNeal"
        }
        body {
            Children unknown. step-son present at death
        }
    }
    child {
        name John Galbraith
        birth -date 6-oct-1884 -location colonsay -ref john1884-birth
        death -date 10-sep-1941 -location 'at sea' -ref john1884-death -note "Supposed drowned."
        body {
            residence in Glasgow, marriage and children uninvestigated.
        }
    }
    child {
        name Euphemia Galbraith
        birth -date 1888 -location colonsay -ref euphemia1888-birth
        death -date 21-dec-1939 -location glasgow -ref euphemia1888-death
        partner {
            name John Slevin
        }
    }
}
    
footnotes {

    donald1853-baptism {
        $opr-ref[b-1853-539-020-0020-0019 "Donald Galbreath"]
    }
    donald1853-marriage {
        $sp-ref-link[m-1879-564-01-0033 0017 "Donald Galbraith" "Christina McPhee"]
    }
    donald1853-death {
        $sp-ref-link[d-1907-539-02-0002 0001 "Donald Galbraith"]
    }
    mcphee-death {
        $sp-ref-link[d-1898-539-02-0005 0002 "Christina Galbraith"]
    }
    hugh1882-birth {
        $sp-ref-link[b-1882-564-01-0295 0099 "Hugh Galbraith"]
    }
    hugh1882-death {
        $sp-ref-link[d-1962-644-11-0908 0303 "Hugh Galbraith"]
    }
    john1884-birth {
        $sp-ref-link[b-1884-539-02-0007 0003 "John Galbraith"]
    }
    john1884-death {
        Marine Returns MIN0043-000-D-0015
    }
    euphemia1888-birth {
        $sp-ref[b-1888-539-02-0001 "Euphemia Galbraith"]
    }
    euphemia1888-death {
        $sp-ref-link[d-1939-644-17-1308 0436 "Euphemia Galbraith"]
    }
}

