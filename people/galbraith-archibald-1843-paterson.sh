name Archibald Galbraith
tags Argyll:Campbeltown glasgow 'dead end'

external {
    familysearch KCR1-MWR
}

birth -date 17-jul-1843
baptism -date 13-aug-1843 -location campbeltown -ref archibald1843-baptism
partner {
    name Mary Paterson
    birth -date 'about 1842'
    marriage -date 10-jun-1870 -location glasgow -ref paterson-marriage
    body {
        The daughter of Charles Paterson and Margaret McCallum.
    }
    child {
        name Mary Galbraith
        birth -date 16-sep-1875 -location campbeltown -ref mary1875-birth
    }
}

body {
    He was a coppersmith.
}
body {
Scotland Census 1871, Shields Place, 3, Govan, Govan Church, Lanarkshire, Scotland:$ref[census-1871]
$csvtable{
First,Last,Role,Age,Year,Birth,Occupation
Archihbald,Galbraith,Head,26,1945,Campbeltown,Coppersmith
Mary,Galbraith,Wife,28,1943,Campbeltown,-
Samuel,Galbraith,Brother,19,1852,Campbeltown,Draper shopman
Margaret,Galbraith,Sister,16,1855,Campbeltown,Shirt maker
}

}
note {
    Marriage records Archibald and Mary lived in Glasgow, but no evidence of children or deaths in
    statutory records.  No additional information found in ancestry.com and familysearch.
}
note {
    A different Archibald Galbraith married a different Mary Paterson in 1865 in Campbeltown.
}
footnotes {
    archibald1843-baptism {
  	    $opr-ref-link[b-1843-507-000-0070-0244 "Archibald Galbreath"]
    }
    paterson-marriage {
        $sp-ref-link[m-1870-644-05-0097 0049 "Archibald Galbraith" "Mary Paterson"]        
    }
    census-1871 {
        https://www.findmypast.com/transcript?id=GBC%2F1871%2F0024906490
    }
}
