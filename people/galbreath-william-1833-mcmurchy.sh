name William Galbraith
tags Campbeltown
birth -date 29-sep-1833 
baptism -date 2-oct-1833 -location Campbeltown -ref william1833-baptism
death -date 28-may-1894 -location campbeltown -ref william1833-death
note {
    Larne, Ireland is almost exactly straight south from Campbeltown across the sea. (Google Maps: https://goo.gl/maps/7TCf3CzZ54MxdBq17)
}
external {
    familysearch K84P-4BG
}

partner {
    name Maria McMurchy
    marriage -date 4-apr-1851 -location IslandMagee/Larne,Ireland -ref marriage
    death -date 12-nov-1893 -location campbeltown -ref maria-mcmurchy-death
    child {
        name Helen Wallace Galbraith
        birth -date 9-Apr-1864 -location campbeltown -ref helen1864-birth
        partner {
            name Donald Bowie
        }
    }
    child galbraith-william-1866-mclarty-mcmillan
    child galbraith-thomas-mcmurchy-1869-mcculloh-mcaulay
    child {
        name David Galbraith
        birth -date 18-aug-1871 -location campbeltown -ref david1871-birth
        death -date 14-mar-1936 -location cathcart -ref david1871-death -note umd
    }
    child {
        name Maria Galbraith
        birth -date 11-apr-1874 -location campbeltown -ref maria1874-birth
        death -date 1876 -location campbeltown -ref maria1874-death
    }

    child galbraith-john-1876-brown-mitchell

    child {
        name Janet McMurchy Galbraith
        birth -date 21-nov-1879 -location campbeltown -ref janet1879-birth
        partner {
            name John Alexander Bryce
        }
    }
}
footnotes {
    william1833-baptism {
        $opr-ref-link[b-1833-507-000-0070-0032 "William Galbreath"]{
            William -- Lawful son of David Galbreath Sawyer and 
            Janet Mitchell, was born 29th Sept + Bapt 2 Oct 1833
        }   
    }
    william1833-death {
        $sp-ref-link[d-1894-507-00-0094 0032 "William Galbraith"]
    }
    marriage {
        two records from Family search https://www.familysearch.org/ark:/61903/1:1:QGVD-29Z8 https://www.familysearch.org/ark:/61903/1:1:FGX1-QBP Source lists William’s father as “David Galbraith”
    }
    maria-mcmurchy-death {
        $sp-ref-link[d-1893-507-00-0153 0051 "Maria McMurtrie"]
        there is a correction associated with the entry.
    }
    helen1864-birth {
        $sp-ref[b-1864-507-00-0093 "Helen Wallace Galbraith"]
    }
    david1871-birth {
        $sp-ref-link[b-1871-507-00-0199 0067 "David Galbraith"]
    }
    david1871-death {
        $sp-ref-link[d-1936-644-19-351 0117 "David Galbraith"]
    }
    maria1874-birth {
        $sp-ref[b-1874-507-00-0097 "Maria Galbraith"]
    }
    maria1874-death {
        $sp-ref[d-1876-507-00-0199 "Maria Galbraith"]
    }
    john1876-birth {
        $sp-ref-link[b-1876-507-00-0101 0034 "John Galbraith"]
    }
    john1876-death {
        $sp-ref-link[d-1953-507-00-0012 0004 "John Galbraith"]
    }
    janet1879-birth {
        $sp-ref-link[b-1879-507-00-0302 0107 "Janet McMurchy Galbraith"]
    }
}
