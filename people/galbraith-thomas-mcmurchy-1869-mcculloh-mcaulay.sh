name Thomas McMurchy Galbraith
birth -date 23-Apr-1869 -location campbeltown -ref thomas1869-birth
death -date 1-jul-1940 -location campbeltown -ref thomas1869-death
external {
    familysearch M1Y9-W41
}
partner {
    name Mary McCulloh
    marriage -date 3-Jun-1896 -location Campbeltown -ref mcculloh-marriage
    death -date 12-Aug-1909 -location Campbeltown -ref mcculloh-death
    child {
        name Duncan Galbraith
        birth -date 23-apr-1891 -location campbeltown -ref duncan1891-birth
    }
    child {
        name William Galbraith
        birth -date 6-oct-1898 -location campbeltown -ref william1898-birth
        death -date 1978 -location campbeltown -ref william1898-death
    }
    child {
        name David Galbraith
        birth -date 21-mar-1904 -location campbeltown -ref david1904-birth
        death -date 1-may-1987 -location campbeltown -ref david1904-death
    }
}
partner {
    name Isabella McAulay
    birth -date 1876
    death -date 13-feb-1941 -location campbeltown -ref mcaulay-death
    marriage -date 8-Jun-1910 -location campbeltown -ref mcaulay-marriage

    child {
        name Maria McMurchy Galbraith
        birth -date 1911 -location campbeltown -ref maria1911-birth
        death -date 1981
    }
    child {
        name Daniel McAulay Galbraith
        birth -date 17-Dec-1913 -location campbeltown -ref daniel1913-birth
        death -date 1975 -location campbeltown -ref daniel1913-death
    }
}

footnotes {
    thomas1869-birth {
        $sp-ref-link[b-1869-507-00-0078 0026 "Thomas McMurchy Galbraith"]
    }
    thomas1869-death {
        $sp-ref-link[d-1940-507-00-0061 0021 "William Galbraith"]
    }
    mcculloh-death {
        $sp-ref-link[d-1909-507-00-0085 0029 "Mary McCulloh"]
    }
    mcculloh-marriage {
        $sp-ref-link[m-1896-507-00-0017 0009 "Thonas Galbraith" "Mary McCulloh"]
        Note mispelling of Thomas. Correction sent to Scotlands People in Feb-2023.
    }
    mcaulay-death {
        $sp-ref-link[d-1941-507-00-0026 0009 "Isabella McAulay"]
    }
    mcaulay-marriage {
        $sp-ref[m-1910-507-00-0018 "Thomas Galbraith" "Isabella McAulay"]
    }
    duncan1891-birth {
        $sp-ref-link[b-1891-507-00-0177 0076 "Duncan Galbraith"]
    }
    william1898-birth {
        $sp-ref-link[b-1898-507-00-0226 0076 "William Galbraith"]
    }
    william1898-death {
        $sp-ref[d-1978-536-00-0060 "William Galbraith"]
    }
    maria1911-birth {
        $sp-ref[b-1911-507-00-158 "Maria McMurchy Galbraith"]
    }
    daniel1913-birth {
        $sp-ref-link[b-1913-507-00-0229 0077 "Daniel McAulay Galbraith"]
    }
    daniel1913-death {
        $sp-ref[d-1975-536-00-0113 "Daniel McAulay Galbraith"]
    }
    david1904-birth {
        $sp-ref-link[b-1904-507-00-0072 0024 "David Galbraith"]
    }
    david1904-death {
        $sp-ref[d-1987-536-00-0036 "David Galbraith"]
    }
}

