name Isabella Galbraith
gender female
tags Argyll:Campbeltown 
external {
    familysearch KC6D-5CN
    findagrave 220562291
}
baptism -date 12-dec-1808 -location Campbeltown -ref isobell-baptism
death -date 18-nov-1880 -location glasgow  -ref isobell-death
todo {
    stub.
}
partner {
   name James Paterson
   gender male
   parent paterson-john-1764-fleeming
   marriage -date 31-jul-1828 -location campbeltown -ref paterson-marriage

   body {
    He worked as a distillery mashman.
   }
}
footnotes {
    isobell-baptism {
        $opr-ref-link[b-1808-507-000-0040-0329 "Isobell Galbeath"]
    }
    isobell-death {
        $sp-ref-link[d-1880-644-08-0813 0271 "Isabella Galbraith"]
    }
    paterson-marriage {
        $opr-ref-link[m-1828-507-000-0060-0285 "Isabella Galbreath" "James Paterson"]
        $blockquote{
            James Paterson labourer and Isabella Galbreath
            both of this parish were married 31st July 1820
        }
    }
}
