name Alexander Galbraith
tags 'Argyll:Saddell and Skipness'
external {
    familysearch G3XR-SNS
    findagrave 172200083
}
birth -date 19-mar-1887 -location carradale -ref alexander1887-birth
death -date 12-nov-1966 -location carradale -ref alexander1887-death

partner {
    name Eliza Tiny Campbell
    marriage -date 22-jun-1910 -location saddell -ref campbell-marriage
    birth -date 21-feb-1886
    death -date 23-nov-1912 -location campbeltown -ref campbell-death 

    child galbraith-john-colin-campbell-1910-macdougall

    child galbraith-alexander-1914-mcdougall

    child galbraith-richard-harris-1918-rooney

    child {
        name Colin Campbell Galbraith
        birth -date 11-aug-1921 -location carradale -ref colin1921-birth
    }
    child {
        name Archibald Cook Galbraith
        birth -date 'about 1926'
        death -date 24-apr-1945 -location glasgow -ref archibald1927-death -note umd
    }
}

footnotes {

    alexander1887-birth {
        $sp-ref[b-1887-531-01-0003 "Alexander Galbraith"]
    }
    campbell-marriage {
        $sp-ref[m-1910-531-01-0001 "Alexander Galbraith" "Eliza Tiny Campbell"]
    }
    campbell-death {
        $sp-ref-link[d-1972-536-00-0116 0116 "Eliza Tiny Galbraith"]
    }
    colin1921-birth {
        $sp-ref-link[b-1921-531-01-0004 0002 "Colin Campbell Galbraith"]
    }
    archibald1927-death {
        $sp-ref-link[d-1945-644-13-0459 0153 "Archibald Cook Galbraith"]
    }
}
