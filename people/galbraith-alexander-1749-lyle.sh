name Alexander Galbraith
gender male
external {
   familysearch KP4H-CMM
}
note {
    Confused with $child-link[galbreath-alexander-1753-flemming]{Alexander Galbreath} and Fleeming of Campbeltown, who immigrated to Peach Bottom, PA.
}

birth -date 1749 -location Bucks,Pennsylvania,USA
death -date 1835 -location Wheat,Roane,Tennessee,USA
partner {
   name Rosannah Lyle
   gender female
   parent {
      name Robert Lyle
      gender male
      partner {
         name Mary Gilliland
         gender female
       }
   }
   birth -date 30-mar-1756 -location 'Forks Township,Northampton,Pennsylvania,USA'
   death -date 19-may-1817 -location 'Anderson,Tennessee,USA'
   marriage -date 'about 1777' -location 'Forks Township,Northampton,Pennsylvania,USA'
}
