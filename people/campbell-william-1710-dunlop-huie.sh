name William Campbell
gender male
baptism -date 19-mar-1710 -location campbeltown
death -date 24-jun-1759 -location campbeltown

tags Argyll:campbeltown campbell

external {
	familysearch K8DV-P63
}

body {

$blockquote{CC/8/57 7. William Campbell sometime merchant in Campbeltown}

$blockquote{Samuel, William and Elizabeth Campbell applied to be decerned executors dative qua nearest in kin to the defunct, who died 24 June 1759. Also com peared $child-link[orr-robert-1730-huie]{Robert Orr}, merchant, and new husband of the relict, Isabella Huie, and Alexander Johnston jun., husband of the eldest daughter, Agnes. In the end, John Campbell, brother gennan to the deceased and tutor in law to the minors, was to give in the inventory. (Testament dative attached.) Par. Campbeltown. 2 July 1761.
}

}
partner {
	name Elizabeth Dunlop
	gender female
	marriage 01-jun-1735 -location campbeltown

	child {
		name Mary Campbell
		gender female
		baptism -date jan-1735 -location campbeltown
	}

	child {
		name Agnes Campbell
		gender female
		baptism -date 13-dec-1736 -location campbeltown
	}

	child {
		name Jean Campbell
		gender female
		baptism -date 14-jan-1739 -location campbeltown
	}
	child {
		name Alexander Campbell
		gender male
		baptism -date 26-jul-1740 -location campbeltown
		death -note dy
	}
	child {
		name Alexander Campbell
		gender male
		birth -date 30-jul-1742 -location campbeltown
	}
	child {
		name James Campbell
		gender male
		birth -date 11-jun-1746
		baptism -date 12-jun-1746 -location campbeltown
	}
	child {
		name Janet Campbell
		gender female
		birth -date 11-jun-1746
		baptism -date 12-jun-1746 -location campbeltown
	}
}
partner {
	name Isabell Huie
	gender female
	birth -date 'say 1727'
	marriage -date 11-feb-1750 -location campbeltown

	body {
		After William's death in 1759, she remarried to $child-link[orr-robert-1730-huie]{Robert Orr}.
	}
}

