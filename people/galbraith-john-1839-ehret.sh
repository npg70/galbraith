name John Galbraith
tags Argyll:Campbeltown
birth -date 1839 -location campbeltown
death -date 15-dec-1908 -location Holborn,London,England -ref john-death
partner {
    name Eliza Ehret
    birth -date 1851 -location Clerkenwell,Middlesex,England
    marriage -date 1869 -ref ehret-marriage
    death -date 1880 -location Kensington,London,England  -ref ehret-death
    body {
        She died between 1877 and 1881.
    }
    child {
        name Janet Galbraith
        birth -date 1869 -location Islington,Middlesex,England
    }
    child {
        name Eliza Galbraith
        birth -date 1871 -location Islington,Middlesex,England
    }
    child {
        name Frederick Galbraith
        birth -date 1874 -location Islington,Middlesex,England
    }
    child {
        name Emily Galbraith
        birth -date 1877 -location Islington,Middlesex,England
    }
}
body {
A "Printer Compositor" (layout / printer), and active trade unionist.

Census 1871 - Nailour Street, Islington, London & Middlesex, England$ref[census-1871]

$csvtable{
First,Last,Role,Age,Year,Place,Occupation
John,Galbraith,Head,30,1841,Scotland,Printer compositor
Eliza,Galbraith,Wife,20,1851,Clerkenwell,-
Janet,Galbraith,Dau,2,1869,Islington,-
Eliza,Galbraith,Dau,0,1871,Islington,-
}

Census 1881 - 4, Dundee Bdgs, Clerkenwell, Holborn, London & Middlesex, England$ref[census-1881]

$csvtable{
First,Last,Role,Age,Year,Place,Occupation
John,Galbraith,Head,30,1841,Scotland,Printer compositor
Janet,Galbraith,Dau,12,1869,Islington,Scholar
Eliza,Galbraith,Dau,10,1871,Islington,Scholar
Frederick,Galbraith,Son,7,1874,Islington,Scholar
Emily,Galbraith,Dau,4,1877,Islington,-
}

Census 1891 -Penny Bank Buildings 4Th Floor, 16, St Johns Square, Clerkenwell, Holborn, London & Middlesex, England$ref[census-1891]

$csvtable{
First,Last,Role,Age,Year,Place,Occupation
John,Galbraith,Head,53,1838,Scotland,Printer compositor
Jessie,Galbraith,Dau,22,1869,London,Human hair worker
Eliza,Galbraith,Dau,20,1871,London,Human hair worker
Emily,Galbraith,Dau,14,1877,London,-
}

Census 1901 - Penny Bank Buildings, 8, Clerkenwell, Holborn, London & Middlesex, England$ref[census-1901]

$csvtable{
First,Last,Role,Age,Year,Place,Occupation
John,Galbraith,Head,63,1838,Scotland,Printer compositor
Jessie,Galbraith,Dau,30,1871,Holloway,Islington,-
Emily,Galbraith,Dau,24,1877,Holloway,Vellum Folder
}

$elink[https://books.google.com/books?id=OV02W1GizPwC&pg=RA24-PA12&lpg=RA24-PA12&dq=John+Galbraith+%22London+Society+of+Compositors%22&source=bl&ots=Xcm8rJeMfg&sig=ACfU3U0giVtAEi0X-5B1HNx9i3bcP8Af5A&hl=en&sa=X&ved=2ahUKEwizpIu-l5z5AhULIEQIHWjAAd0Q6AF6BAgVEAI#v=onepage&q=John%20Galbraith%20%22London%20Society%20of%20Compositors%22&f=false]{Additional obit} on Google Books.

}

footnotes {
    john-death {
        https://www.findmypast.com/transcript?id=BMD%2FD%2F1908%2F4%2FAZ%2F000125%2F100&tab=this
    }
    census-1871 {
        https://www.findmypast.com/transcript?id=GBC%2F1871%2F0001693819&tab=this
    }
    census-1881 {
        https://www.findmypast.com/transcript?id=GBC%2F1881%2F0001699939&tab=this
    }
    census-1891 {
        https://www.findmypast.com/transcript?id=GBC%2F1891%2F0001981602&tab=this
    }
    census-1901  {
        https://www.findmypast.com/transcript?id=GBC%2F1901%2F0007176067&tab=this
    }
    ehret-marriage {
        Due to strange UK system, there are two indirect records.  First record is for $elink[https://www.findmypast.com/transcript?id=BMD%2FM%2F1869%2F1%2FAZ%2F000079%2F140&tab=this]{John Galbraith} which lists the marriage in the first quarter of 1869, in Islington, London, Volume 1B, Page 337.

        The second record is for $elink[https://www.findmypast.com/transcript?id=BMD%2FM%2F1869%2F1%2FAZ%2F000067%2F139&tab=this]{Eliza Ehret}. Due to a transcription error, it says the record is on Page 317.  FindMyPast has been notified on 24-Sep-2024.
    }

    ehret-death {
        https://www.findmypast.com/transcript?id=BMD%2FD%2F1880%2F1%2FAZ%2F000136%2F337&tab=this

        Gives birth year of 1852.  Note that in 1881 Census John G- is listed as a widower.
    }
}
