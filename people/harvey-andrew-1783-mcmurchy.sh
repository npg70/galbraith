name Andrew Harvey
external {
    familysearch M1TP-B45
    findagrave 274553791
    wikitree Harvey-12681
    ancestry tree/12079504/person/282299027690
}

birth -date 23-feb-1783
baptism -date 26-feb-1783 -location campbeltown -ref andrew1783-baptism
death -date 14-sep-1849 -location avondale,hamilton,ohio,usa
partner {
    name Elizabeth McMurchy
    baptism -date 28-jan-1786 -location campbeltown
    marriage -date 6-apr-1809 -location campbeltown -ref mcmurchy-marriage
    body {
        She was the daugher of William McMurchy and $child-link[clark-janet-1747-mcmurchy]{Janet Clark}.
    }
    child {
        name Janet Harvey
        birth -date 28-feb-1810
        baptism -date 28-feb-1810 -location campbeltown -ref janet1810-baptism
        death -date 01-apr-1893 -location 'Hanover,Jefferson,Indiana,USA'
        partner {
            name William McMurchy
        }
        partner {
            name Hugh Hamilton Young
        }
        body {
            Immigrated 17-Sep-1821 in Philadelphia.
        }
    }
    child {
        name James Harvey
        birth -date 03-oct-1811
        baptism -date 09-oct-1811 -location campbeltown -ref james1811-baptism
        death -note 'died young'
    }
    child {
        name William Harvey
        birth -date 01-mar-1813
        baptism -date 02-mar-1813 -location campbeltown -ref william1813-baptism
        death -date 04-aug-1822
    }
    child {
        name Catherine Harvey
        birth -date 22-feb-1815
        baptism -date 01-mar-1815 -location campbeltown -ref catherine1815-baptism
        death -date 17-dec-1898 -location cincinnati,hamilton,ohio,usa
        partner {
            name Robert Young
            birth -date 1802
            death -date 1877
        }
    }
    child {
        name Elizabeth Harvey
        birth -date 05-mar-1817
        baptism -date 12-mar-1817 -location campbeltown -ref eliz1817-baptism
        death -date 31-jan-1818
    }
    child {
        name Elizabeth Harvey
        birth -date 10-jun-1820
        baptism -date 14-jun-1820 -location campbeltown -ref eliz1820-baptism
        death -date 04-sep-1858 -location 'Chalmers, McDonough, Illinois, USA'
        partner {
            name William Hunter
            birth -date 1816
            death -date 1889
        }
    }
    child {
        name James Harvey
        birth -date 04-aug-1822 -location 'Hamilton County, Ohio, USA'
        death -date 29-apr-1902 -location 'Bantam,Clermont,Ohio,USA'
    }
    child {
        name Martha Harvey
        birth -date 1824
        death -date 1825
    }
    child {
        name Andrew Harvey
        birth -date 8-Aug-1826 -location 'Cincinnati, Hamilton, Ohio, United States'
        death -date 11-May-1890 -location 'Cincinnati, Hamilton, Ohio, United States'
        partner {
            name Emma E. Brooks
            birth -date 1835
            death -date 1822
        }
    }
    child {
        name William Harvey
        birth -date 10-dec-1828 -location 'Avondale, Hamilton, Ohio, USA'
        death -date 24-oct-1897 -location 'Walnut Hills, Montgomery, USA'
        partner {
            name Sarah Jane Felicia Kemper
            birth -date 1837
            death -date 1914
        }
    }
}
footnotes {
    andrew1783-baptism {
        $opr-ref-link[b-1783-507-000-0020-0065 "Andrew Harvey"]{
            Andrew | Lawful son to Jas. Harvey Farmer in Park & Kath-erine
            Galbreath born 23rd & Baptd 26th Febry 1783.
        }
    }
    mcmurchy-marriage {
        $opr-ref-link[m-1809-507-000-0050-0086 "Andrew Harvie" "Elizabeth MacMurchy"]{
            Andrew Harvie Farmer Peninver and Elizabeth MacMurchy
            daughter to William MacMurchy Farmer in Moy, both of this
            parish were married the 6th April 1809.
        }
    }
    janet1810-baptism {
        $opr-ref-link[b-1810-507-000-0040-0348 "Janet Harvey"]{
            Janet | Lawful Daughter to Andrew Harvey Farmer Peninver and
            Elizabeth MacMurchy born 27th bapt 28th Feb 1810 Named Janet
        }
    }
    james1811-baptism {
        $opr-ref[b-1811-507-000-0040-0375 "James Harvie"]
    }
    william1813-baptism {
        $opr-ref[b-1813-507-000-0040-0395 "William Harvie"]
    }
    catherine1815-baptism {
        $opr-ref-link[b-1815-507-000-0040-0426 "Catharine Harvie"]{
            Catharine | Lawful Daughter to Andrew Harvie farmer Peninver
            and Elizabeth MacMurchy born 22 Feb & bapt 1st March 1815. 
            Named Catharine
        }
    }
    eliz1817-baptism {
        $opr-ref-link[b-1817-507-000-0040-0458 "Elizabeth Harvey"]{
            Elizabeth | Lawful Daughter to Andrew Harvey, Farmer Peninver
            and Elizabeth MacMurchy born 5th bapt 12th March 1817.
            Elizabeth
        }
    }
    eliz1820-baptism {
        $opr-ref-link[b-1820-507-000-0060-0019 "Elizabeth Harvie"]{
            Elizabeth Lawful Daughter of Andrew Harvey + Elizabeth McMurchy
            in Peninver. Born 10th + Bapt 14th June 1820. Elizabeth
        }
    }    
}
