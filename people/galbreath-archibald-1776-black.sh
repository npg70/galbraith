name Archibald Galbreath

birth -date 'about 1776'

external {
    familysearch K8VW-HKP
}
body {
    He perhaps died prior to 1851 since he does not appear in the census of the time.
}

body {
    He was a shoemaker, according to various death records of his children.
}
note {
	There is another Archibald Galbreath who married Jean Black, 1833 in North Berwick
    }
partner {
    name Jean Black
    birth -date 'about 1774'
    marriage -date 23-Jul-1800 -location 'Killean and Kilchenzie' -ref black-marriage
    death -date 17-feb-1862 -location 'Killean and Kilchenzie' -ref black-death
    body {
        She was the daughter of Dugald Black and Flora Galbraith.
    }
    child {
        name Margaret Galbreath
        baptism -date 23-mar-1802 -location 'killean and kilchenzie' -ref margaret1802-baptism
        death -date 13-nov-1871 -location 'killean and kilchenzie' -ref margaret1802-death
        partner {
            name Duncan McIntosh
        }
    }
    child {
        name John Galbreath
        baptism -date 27-may-1804 -location 'killean and kilchenzie' -ref john1804-baptism
    }
    child galbraith-archibald-1807-mcmillan
    child {
        name Flora Galbreath
        baptism -date 23-jul-1809 -location 'killean and kilchenzie' -ref flora1809-baptism
    }
    child {
        name Mary Galbreath
        baptism -date 28-aug-1811 -location 'killean and kilchenzie' -ref mary1811-baptism
        death -date 23-feb-1800 
        partner {
            name Robert Greenlees
        }
    }
    child {
        name Betty Galbreath
        baptism -date 30-jun-1813 -location 'killean and kilchenzie' -ref betty1813-baptism
    }
    child galbreath-duncan-1815-mcfater
    child {
        name Dugald Galbreath
        baptism -date 14-sep-1817 -location 'killean and kilchenzie' -ref dugald1817-baptism
    }
}

footnotes {
    black-marriage {
        $opr-ref-link[m-1800-519-000-0010-0342 "Archibald Galbreath" "Jean Black"]
    }
    black-death {
        $sp-ref-link[d-1862-519-00-0011 0004 "Jean Galbraith"]
    }
    margaret1802-baptism {
        $opr-ref[b-1802-519-000-0010-0162 "Margaret Galbreath"]
    }
    margaret1802-death {
        $sp-ref-link[d-1871-519-00-0022 0008 "Margaret Galbraith"]
    }
    john1804-baptism {
        $opr-ref[b-1804-519-000-0010-0180 "John Galbreath"]
    }
    flora1809-baptism {
        $opr-ref[b-1809-519-000-0010-0229 "Flora Galbreath"]
    }
    mary1811-baptism {
        $opr-ref[b-1811-519-000-0010-0248 "Mary Galbreath"]
    }
    betty1813-baptism {
        $opr-ref[b-1813-519-000-0010-0264 "Betty Galbreath"]
    }
    dugald1817-baptism {
        $opr-ref-link[b-1817-519-000-0010-0294 "Dugald Galbreath"]{
         | Sept | 14 | Dugald -- Lawful son to Arch Galbreath and Jean Black Achanahalder
        }
    }
}

