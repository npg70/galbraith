name Archibald McNair
gender male
tags Argyll:Campbeltown argyll:chisken
external {
    familysearch LJNB-3G4
    wikitree McNair-1083
    ancestry tree/12079504/person/-339238061
}
confused-with  mcnair-archibald-1759-mitchell
note {
    Not in the 1792 Census.
}
body {
    He was a farmer at Chisken/Shisken.
}
baptism -date 19-nov-1753 -location campbeltown -ref baptism
partner {
   name Jean Mitchell
   marriage -date 02-jul-1784 -location campbeltown -ref mitchell-marriage

   child {
        name Hugh McNair
        gender male
        baptism -date 24-sep-1784 -location campbeltown -ref hugh1784-baptism
        death -note dy
    }
    child mcnair-robert-1785-smith
   child {
        name Mary McNair
        gender female
        birth -date 22-oct-1787
        baptism -date 25-oct-1787 -location campbeltown -ref mary1787-baptism
        death -note dy
    }
    child {
        name James McNair
        gender male
        baptism -date 13-jan-1790 -location campbeltown -ref james1790-baptism
        death -date 1853 -location cincinnati,hamiliton,ohio,usa
        partner dunlop-kate-1783-harvey-mcnair
    }
    child {
        name Hugh McNair
        gender male
        birth -date 26-may-1791
        baptism -date 29-may-1791 -location campbeltown -ref hugh1791-baptism
        death -date 10-dec-1811 -location campbeltown
    }
    child mcnair-janet-1793-porter-mcmurchy

    child {
        name John McNair
        gender male
        baptism -date 11-dec-1794 -location campbeltown -ref john1794-baptism
        body {
            Confused with $child-link[mcnair-john-1793-mckenzie]{John McNair}, son of $child-link[mcnair-archibald-1759-mitchell]{Archibald McNair} and $i{Margaret} Mitchell.
        }
    }

    child mcnair-archibald-1796-mitchell
    child {
        name William McNair
        gender male
        birth -date 19-apr-1799
        baptism -date 22-apr-1799 -location campbeltown -ref william1799-baptism
        death -note dy
    }
    child mcnair-mary-1800-harvey-bowie
    child mcnair-william-1807-mitchell

}
footnotes {
    baptism {
       $opr-ref[b-1753-507-000-0011-0288 "Archibald McNair"]
       $opr-ref[b-1753-507-000-0010-0214 "Archibald McNair"] 
    }
    mitchell-marriage {
        $opr-ref-link[m-1784-507-000-0020-0168 "Archibald McNair" "Jean Mitchell"]{
            Archibald McNair & Jean Mitchell both of this parish were contracted
            2nd July & married
        }  
    }
    hugh1784-baptism {
        Indexed as "Hugh McLiver" since it's written then crossed out in the register.
        $opr-ref-link[b-1784-507-000-0020-0091 "Hugh McNair"]
    }
    mary1787-baptism {
        $opr-ref-link[b-1787-507-000-0020-0099 "Mary McNair"]
    }
    james1790-baptism {
        $opr-ref[b-1790-507-000-0020-0116 "James McNair"]
    }
    hugh1791-baptism {
        $opr-ref[b-1791-507-000-0020-0131 "Hugh McNair"]
    }
    john1794-baptism {  
        $opr-ref-link[b-1794-507-000-0040-0121 "John McNair"]{
            John Lawful son to Archd McNair and Jean Mitchell of Shiskon Bapt [???]
        }
    }
    william1799-baptism {
        $opr-ref[b-1799-507-000-0040-0183 "William McNair"]
    }
}
