name Archibald Galbreath
baptism -date 22-apr-1807 -location 'killean and kilchenzie' -ref archibald1807-baptism
death -date 16-Jan-1891 -location 'killean and kilchenzie' -ref archibald1807-death
burial -name 'Putechan Cemetery'
external {
    findagrave 161421238
}
partner {
    name Betty McMillan
    marriage -date 17-jun-1862 -location 'killean and kilchenzie' -ref mcmillan-marriage
    child galbraith-archibald-1865-robertson
    child {
        name Edward Galbraith
        birth -date 21-apr-1863 -location 'killean and kilchenzie' -ref edward1863-birth
        death -date 29-jun-1921 -location 'killean and kilchenzie' -ref edward1863-death
        partner {
            name Elizabeth McDonald
            marriage -date 6-jun-1907 -location 'killean and kilchenzie' -ref edward1863-marriage
        }
    }
}
footnotes {
    archibald1807-baptism {
        $opr-ref[b-1807-519-000-0010-0208 "Archibald Galbreath"]
    }
    archibald1807-death {
        $sp-ref-link[d-1891-519-00-0001 0001 "Archibald Galbraith"]{
            1. Archibald Galbraith, Shoemaker (Married to Betty McMillan);
            Death: 1891 January Sixteenth, age 83
            Father: Archibald Galbraith, Shoemaker (deceased)
            Mother: Jean Galbraith M.S. Black (deceased)
            Present: Edward Galbraith, Son
        }
    }
    mcmillan-marriage {
        $sp-ref-link[m-1862-519-00-0004 0002 "Archibald Galbraith "Elizabeth McMillan"]
    }
    edward1863-birth {
        $sp-ref-link[b-1863-519-00-0029 0010 "Edward Galbreath"]
    }
    edward1863-death {
        $sp-ref-link[d-1921-519-00-0009 0003 "Edward Galbraith"]
    }
    edward1863-marriage {
        $sp-ref-link[m-1907-519-00-0003 0002 "Edward Galbraith" "Elizabeth MacDonald"]
    }
}

