name John Galbraith
tags Glassary
external {
    familysearch M15Y-X2L
    findagrave 182029989
}
todo {
    transcribe census info (see footnotes)
}
todo {
    research post-1855 children
}
baptism -date 8-apr-1826 -location glassary -ref john1826-baptism
death -date 28-jun-1905 -location glassary -ref john1826-death
partner {
    name Catherine McAlpin
    birth -date 1825
    marriage -date 5-feb-1850 -location glassary -ref mcalpin-marriage
    death -date 2-oct-1875 -location glassary -ref mcalpin-death
    child {
        name Mary Galbraith
        baptism -date 22-nov-1850 -location glassary -ref mary1850-baptism
    }
    child galbraith-donald-1852-cleaver-cameron
    child {
        name Isabella Galbraith
        baptism -date 4-nov-1854 -location glassary -ref isabella1854-baptism
    }
    child {
        name Catherine Galbraith
    }
    child {
        name Margaret Galbraith
    }
}

footnotes {
    john1826-baptism {
        $opr-ref[b-1826-511-000-0030-0046 "John Galbreath"]
    }
    john1826-death {
        $sp-ref-link[d-1905-511-00-0016 0006 "John Galbraith"]
    }
    mcalpin-marriage {
        $opr-ref[m-1850-511-000-0030-0366 "John Galbraith" "Catherine MacAlpine"]
    }
    mcalpin-death {
        $sp-ref-link[d-1875-511-00-0015 0005 "Catherine Galbraith"] 
    }
    mary1850-baptism {
        $opr-ref[b-1850-511-000-0030-0244 "Mary Galbraith"]
    }
    isabella1854-baptism {
        $opr-ref[b-1854-511-000-0030-0292 "Isabella Galbraith"]
    }
    census1851 {
        https://www.findmypast.com/transcript?id=GBC%2F1851%2F0019270522
    }
    census1861 {
        https://www.findmypast.com/transcript?id=GBC/1861/0022452626&expand=true
    }
    census1871 {
        https://www.findmypast.com/transcript?id=GBC%2F1871%2F0023457583
    }
}
