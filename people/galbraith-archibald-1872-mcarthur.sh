name Archibald Galbraith
tags Argyll:Campbeltown glasgow
external {
    familysearch 93R3-PBC
}
birth -date 11-jun-1872 -location glasgow -ref archibald1872-birth
death -date 7-jun-1943 -location govan,glasgow -ref archibald1872-death
partner {
    name Mary McArthur
    birth -date 13-aug-1870 -location campbeltown
    marriage -date 29-nov-1889 -location campbeltown -ref mcarthur-marriage
    child {
        name Mary McFayden Galbraith
        birth -date 9-mar-1890 -location campbeltown -ref mary1890-birth
    }
    child {
        name Flora Galbraith
        birth -date 12-feb-1892 -location campbeltown -ref flora1892-birth
    }
    child {
        name Alexander Galbraith
        birth -date 21-feb-1903 -location campbeltown -ref alexander1903-birth
    }
    child {
        name Margaret Galbraith
        birth -date 10-feb-1907 -location campbeltown -ref margaret1907-birth
    }
}
footnotes {
    archibald1872-birth {
        $sp-ref-link[b-1872-644-07-1146 0382 "Archibald Galbraith"]       
    }
    archibald1872-death {
        $sp-ref-link[d-1943-644-17-0576 0192 "Archibald Galbraith"]
    }
    mcarthur-marriage {
        $sp-ref-link[m-1889-507-00-0048 0024 "Archibald Galbraith" "Mary McArthur"]
    }
    mary1890-birth {
        $sp-ref-link[b-1890-507-00-0055 0019 "Mary McFadyen Galbraith"]
    }
    flora1892-birth {
        $sp-ref-link[b-1892-507-00-0036 0012 "Flora Galbraith"]
    }
    alexander1903-birth {
        $sp-ref-link[b-1903-507-00-0048 0016 "Archibald Galbraith"]
    }
    margaret1907-birth {
        $sp-ref-link[b-1907-507-00-0034 0012 "Margaret Galbraith"]
    }
}
