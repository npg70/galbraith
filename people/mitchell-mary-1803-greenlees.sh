name Mary Mitchell
gender female
tags 'Argyll:Killean and Kilchenzie'
baptism -date 12-sep-1803 -location 'Killean and Kilchenzie' -ref mary1803-baptism
external {
    ancestry tree/12079504/person/282659540795
    findagrave 53401235
}
note {
    Confused with $child-link[mcnair-archibald-1796-mitchell]{Mary Mitchell} who married Archibald McNair.
}
partner {
    name Robert Greenlees
    gender male
    baptism -date 02-sep-1799 -location Killean -ref robert1799-baptism
    marriage -date 04-mar-1823 -location Campbeltown -ref robert1799-marriage
}

footnotes {
    mary1803-baptism {
        $opr-ref-link[b-1803-519-000-0010-0174 "Mary Mitchel"]{
            $i{1803} Sep | 12 | Mary Lawful Daughter to 
            James Mitchel Ballergie
        }
    }
    robert1799-baptism {
        $opr-ref-link[b-1799-519-000-0010-0136 "Robert Greenlies"]{
            Sept | 2 | Robert Lawful son to Robt. Greenlies
            & Sarah Templeton. Tighnamorle[?]
        }
    }
    robert1799-marriage {
        $opr-ref-link[m-1824-507-000-0060-0260 "Robert Greenlees" "Mary Mitchell"]{
            Robert Greenlees Farmer in Craigs + Mary Mitchell at Ballery
            were married the 4th March 1824
        }
    }
}
