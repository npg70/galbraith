name Archibald Galbreath
tags 'South Knapdale'
baptism -date 28-nov-1837 -location 'south knapdale' -ref archibald1837-baptism
death -date 19-jan-1936 -location 'south knapdale' -ref archibald1837-death
todo {
parents:  investigate if any grandchildren
}
external {
    familysearch G2TQ-FS1
}
partner {
    name Agnes Morrison
    marriage -date 3-apr-1867 -location glasgow -ref marriage

    child {
        name Donald Galbraith
        birth -date 22-aug-1868 -location lochgilphead -ref donald1868-birth
        death -date 7-jun-1956 -location paisley -ref donald1868-death
        partner {
            name Jeanie Murphy
        }
        partner {
            name Marion Brodie McKellar
        }
    }
    child {
        name Jessie Galbraith
        birth -date 1871 -location lochgilphead -ref jessie1871-birth
    }
    child {
        name Neil Galbraith
        birth -date 19-aug-1873 -location 'south knapdale' -ref neil1873-birth
        death -date 26-may-1958 -location 'kilmore and kilbride' -ref neil1873-death
        partner {
            name Mary Munro
        }
    }
    child {
        name Isabella Galbraith
        birth -date 1875 -location 'south knapdale' -ref isabella1875-birth
    }
    child {
        name Mary Galbraith
        birth -date 1877 -location 'south knapdale' -ref mary1877-birth
    }
    child {
        name Archibald Galbraith
        birth -date 5-may-1879 -location 'south knapdale' -ref archibald1879-birth
        death -date 9-aug-1921  -location 'biggar, lanark' -ref archibald1879-death
        partner {
            name Jean Anderson
        }
    }
    child {
        name Agnes Galbraith
        birth -date 1881 -location 'south knapdale' -ref agnes1881-birth
    }
}

footnotes {

    archibald1837-death {
        $sp-ref-link[d-1936-533-01-0001 0001 "Archibald Galbraith"]
    }
    marriage {
        $sp-ref[m-1867-644-09-0138 0069 "Archibald Galbraith" "Agnes Morrison"]
    }
    donald1868-birth {
        $sp-ref-link[b-1868-526-00-0086 0029 "Donald Galbraith"]
    }
    donald1868-death {
        $sp-ref-link[d-1956-573-01-0724 0242 "Donald Galbraith"]
    }
    jessie1871-birth {
        $sp-ref[b-1871-526-00-0017 "Jessie Galbraith"]
    }
    neil1873-birth {
        $sp-ref-link[b-1873-533-00-0006 0002 "Neil Galbraith"]
    }
    neil1873-death {
        $sp-ref-link[d-1958-523-00-0075 0025 "Neil Galbraith"]
    }
    isabella1875-birth {
        $sp-ref[b-1875-533-00-0003 "Isabella Galbraith"]
    }
    mary1877-birth {
        $sp-ref[b-1898-533-00-0004 "Mary Galbraith"]
    }
    archibald1879-birth {
        $sp-ref-link[b-1879-533-00-0005 0002 "Archibald Galbraith"]
    }
    archibald1879-death {
        $sp-ref-link[d-1921-623-00-0025 0009 "Archibald Galbraith"]
    }
    agnes1881-birth {
        $sp-ref[b-1881-533-00-0004 "Agnes Galbraith"]
    }
}
